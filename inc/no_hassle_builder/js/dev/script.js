/* globals ContentTools, ContentEdit, ContentSelect, nhbParams, nhbEditor, HS */
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	// Auto-start nhb if the localStorage key is set.
	var starterInterval;
	if ( localStorage ) {
		if ( localStorage.getItem( 'nhb-open-' + nhbParams.post_id ) ) {
			localStorage.removeItem( 'nhb-open-' + nhbParams.post_id );
	...
	// Auto-start nhb if there is a hash.
	if ( window.location.hash ) {
		if ( window.location.hash.indexOf( '#nhb-edit' ) === 0 ) {
			starterInterval = setInterval( function() {
				if ( document.querySelector( '.ct-widget--active' ) ) {
					document.querySelector('.ct-ignition__button--edit').dispatchEvent( new CustomEvent( 'click' ) );
					clearInterval( starterInterval );
	...
	document.addEventListener( 'click', function(ev) {
		if ( ev.target.getAttribute( 'id' ) === 'nhb-help-docs' && typeof HS !== 'undefined' && typeof HS.beacon !== 'undefined' && HS.beacon.open !== 'undefined' ) {
			if ( HS.beacon.open ) {
				HS.beacon.open();
			} else {
				window.open( 'http://docs.pagebuildersandwich.com/', '_blank' );
			}
		} else if ( ev.target.getAttribute( 'id' ) === 'nhb-help-docs' ) {
			window.open( 'http://docs.pagebuildersandwich.com/', '_blank' );
		// } else if ( ev.target.getAttribute( 'id' ) === 'nhb-help-replay-tour' ) {
			// window.nhbPlayTour();
		}
	});

	// Docs button
	document.querySelector( '#wp-admin-bar-nhb_help_docs' ).addEventListener( 'click', function(ev) {
	...
	/**
	 * Editor admin bar buttons become hovered when turning on nhb. This stops
	 * it by turning pointer-events to none temporarily after nhb starts.
	 */
	editor.bind('start', function() {
	...
	editor.bind('save', function (regions) {
	    ...
	    for (name in regions) {
	        if (regions.hasOwnProperty(name)) {
				...
				html = wp.hooks.applyFilters( 'nhb.save', html );

	            payload.append(name, html);
	        }
	    }
		payload.append( 'action', 'gambit_builder_save_content' );
		payload.append( 'save_nonce', nhbParams.nonce );
		payload.append( 'post_id', nhbParams.post_id );

		// Change the post status.
		// This is filled up by the other save buttons in _content-tools-UI.js
		if ( nhbParams.new_post_status ) {
			payload.append( 'post_status', nhbParams.new_post_status );
			nhbParams.new_post_status = undefined;
		}

		// If there are pseudo element styles that need to be saved, include those.
		var styles = '';
		if ( document.querySelector( 'style#nhb-style' ) ) {
			styles = document.querySelector( 'style#nhb-style' ).innerHTML;
		}
		payload.append( 'style', styles );

		wp.hooks.doAction( 'nhb.save.payload', payload );
	    ...
	    xhr.open( 'POST', nhbParams.ajax_url );
	    xhr.send( payload );

		// Allow others to do something after saving.
		wp.hooks.doAction( 'nhb.save.payload.post', payload );
	});
	...
	var _showColumnOutlines = _.debounce(function() {
		if ( _isShiftDown ) {
			document.querySelector('html').classList.add('nhb-show-outlines');
		}
	}, 800);
	...
	var hideColumnOutlines = function(e) {
		if ( e.which === 16 ) {
			_isShiftDown = false;
			document.querySelector('html').classList.remove('nhb-show-outlines');
		}
	};


	editor.bind('start', function() {
		document.querySelector('html').classList.add('nhb-editing');
		document.querySelector('html').classList.add('theme-' + nhbParams.theme_name);

		try {
			this.regions()['main-content'].children[0].focus();
		} catch (e) {}
		this.domRegions()[0].classList.add('nhb-editing');
		...
		// window._nhbBodyTransitionIntervalNum = 0;
		// clearInterval( window._nhbBodyTransitionInterval );
		// window._nhbBodyTransitionInterval = setInterval(function() {
		// 	window._nhbBodyTransitionIntervalNum++;
		// 	window.dispatchEvent( new CustomEvent( 'resize' ) );
		// 	if ( window._nhbBodyTransitionIntervalNum >= 30 ) {
		// 		clearInterval( window._nhbBodyTransitionInterval );
		// 	}
		// }, 16);
	...
	editor.bind('stop', function() {
		this.domRegions()[0].classList.remove('nhb-editing');
		document.querySelector('html').classList.remove('nhb-editing');
		document.querySelector('html').classList.remove('theme-' + nhbParams.theme_name);
	...
	ContentEdit.Root.get().bind( 'focus', function( element ) {
		document.body.setAttribute( 'data-nhb-focused', element.constructor.name );
	} );

	ContentEdit.Root.get().bind( 'blur', function() {
		document.body.removeAttribute( 'data-nhb-focused' );
	} );
	 ...
	 */
	editor.bind('start', function() {
		var element = document.querySelector( '.nhb-main-wrapper' );
		...
		while ( element && element.tagName !== 'body' ) {
			...
			while ( curElement.previousSibling ) {
				...
				if ( curElement.previousSibling.querySelector( '.nhb-title-editor' ) ) {
					curElement = curElement.previousSibling;
					continue;
				}

				if ( curElement.previousSibling.classList ) {
					curElement.previousSibling.classList.add( 'nhb-ignore-while-editing' );
				}
				curElement = curElement.previousSibling;
		    }
	...
	editor.bind( 'stop', function() {
		var elements = document.querySelectorAll( '.nhb-ignore-while-editing' );
		if ( elements ) {
			Array.prototype.forEach.call( elements, function( el ) {
				el.classList.remove( 'nhb-ignore-while-editing' );
			});
		}
	} );
	...
	document.body.addEventListener('mousedown', function(e) {
		if ( e.target.classList.contains( 'ct-ignition__button--confirm') || ( e.target.parentNode && e.target.parentNode.classList.contains( 'ct-ignition__button--confirm' ) ) ) {
			window.nhbEditor.updateModifiedContent();
		}
	});
	...
	document.body.addEventListener('mousedown', function(e) {
		...
		if ( window.nhbSelectorMatches( e.target, '.wp-core-ui *' ) ) {
			return;
		}

		// Check if the one being clicked is the page title, we will allow it to be edited.
		var isInTitle = window.nhbSelectorMatches( e.target, '.nhb-title-editor, .nhb-title-editor *' );

		// Only do this when we are editing.
		if ( editorArea.classList.contains( 'nhb-editing' ) && isInEditorSemiRoot && ! isEditorArea && ! isInEditorArea && ! isInTitle ) {
...
ContentEdit.Root.get().bind('drop', function () {
	...
	if ( window.nhbFixRowWidths ) {
		window.nhbFixRowWidths();
	}
});

// From http://davidwalsh.name/element-matches-selector
window.nhbSelectorMatches = function(el, selector) {
...
window.nhbEditor.getToolUI = function( name ) {
 ...
 */
window.nhbEditor.updateModifiedContent = function() {
	...
	for ( var i in regions ) {
		if ( regions.hasOwnProperty( i ) ) {
			...
			window.nhbEditor.updateModifiedContentRecursive( regions[ i ] );
		}
	}
};
 ...
 */
window.nhbEditor.updateModifiedContentRecursive = function( element ) {
	...
	if ( children ) {
		for ( var k = 0; k < children.length; k++ ) {
			...
			if ( typeof ctElement.content !== 'undefined' && ctElement.content.html() !== ctElement._domElement.innerHTML ) {
				...
				window.nhbEditor.updateElementContent( ctElement, ctElement._domElement.innerHTML );
			}

			window.nhbEditor.updateModifiedContentRecursive( ctElement );
		}
	}
};
 ...
 */
window.nhbEditor.updateElementContent = function( element, newContent ) {
 ...
 */
window.nhbEditor.isEditing = function() {
	var editorArea = document.querySelector('[data-name="main-content"]');
	if ( ! editorArea ) {
		return false;
	}
	return editorArea.classList.contains( 'nhb-editing' );
};
 ...
 */
window.nhbEditor.getUniqueClassName = function( domElement ) {
	if ( domElement.getAttribute( 'class' ) ) {
		var classes = domElement.getAttribute( 'class' );
		var matches = classes.match( /nhbguid-\w+/ );
		if ( matches ) {
			return matches[0];
		}
	}
 ...
 * Mostly used for adding pseudo element styles using addPseudoElementStyles
 */
window.nhbEditor.removeUniqueClassName = function( domElement ) {
	if ( domElement.getAttribute( 'class' ) ) {
		var classes = domElement.getAttribute( 'class' );
		var matches = classes.match( /nhbguid-\w+/ );
		if ( matches ) {
 ...
 * Mostly used for adding pseudo element styles using addPseudoElementStyles
 */
window.nhbEditor.generateUniqueClassName = function() {
	var name;
	do {
		name = 'nhbguid-' + window.nhbEditor.generateHash();
 ...
 */
window.nhbEditor.generateHash = function() {
 ...
 * Similar to window.nhbEditor.addPseudoElementStyles, except that this doesn't
 * perform any duplication checks and just directly adds the styles.
 * @param string styles The styles to add.
 * @return string The added styles.
 */
window.nhbEditor.addPseudoElementStylesRaw = function( styles ) {

	// Create style tag if it doesn't exist yet.
	var styleTag = document.querySelector( 'style#nhb-style' );
	if ( ! styleTag ) {
		styleTag = document.createElement( 'style' );
		styleTag.setAttribute( 'id', 'nhb-style' );
...
/**
 * Adds a pseudo element style. Adds the style tag used by nhb if it doesn't exist yet.
 ...
 */
window.nhbEditor.addPseudoElementStyles = function( selector, styles ) {

	// Clean up first.
	var selectorStillExists = window.nhbEditor.removePseudoElementStyles( selector, Object.keys( styles ) );
 ...
 */
window.nhbEditor.getPseudoElementStyles = function( selector, style ) {
 ...
 */
window.nhbEditor.removePseudoElementStyles = function( selector, styles ) {
 ...
 */
window.nhbEditor.convertSVGToBackgroundImage = function( svgElement ) {
...
window.nhbEditor.isCtrlDown = false;
window.nhbEditor.isShiftDown = false;
window.addEventListener( 'DOMContentLoaded', function() {
	document.addEventListener( 'keydown', function( ev ) {
		if ( ev.ctrlKey || ev.metaKey ) {
			window.nhbEditor.isCtrlDown = true;
		}
		if ( ev.shiftKey ) {
			window.nhbEditor.isShiftDown = true;
		}
	} );
...
window.addEventListener( 'DOMContentLoaded', function() {
	document.body.addEventListener( 'click', function( ev ) {
		if ( window.nhbEditor.isEditing() ) {
			if ( ev.target.tagName === 'A') {
				if ( window.nhbSelectorMatches( ev.target, '.nhb-main-wrapper *' ) ) {
					ev.preventDefault();
				}
			}
		}
...
/**
 * Fires nhb.ct.ready when the edit button becomes available.
 */
window.addEventListener( 'DOMContentLoaded', function() {
	if ( ! document.querySelector( '[data-name="main-content"]' ) ) {
		return;
	...
	var starterInterval = setInterval( function() {
		if ( document.querySelector( '.ct-widget--active' ) ) {
			wp.hooks.doAction( 'nhb.ct.ready' );
			clearInterval( starterInterval );
		}
	}, 200);
});

/* globals ContentTools, nhbParams */
...
ContentTools.IgnitionUI.prototype.mount = function() {
	...
	document.querySelector( '#wp-admin-bar-gambit_builder_save_options #nhb-save-publish' ).addEventListener( 'click', function(ev) {
		ev.preventDefault();
		nhbParams.new_post_status = 'publish';
		this._domConfirm.click();
		document.querySelector('#nhb-save-button').setAttribute( 'data-current-post-type', 'publish' );
		this._domConfirm.querySelector('.ab-item').childNodes[1].nodeValue = nhbParams.labels.save_and_update;
	}.bind(this));
	document.querySelector( '#wp-admin-bar-gambit_builder_save_options #nhb-save-draft' ).addEventListener( 'click', function(ev) {
		ev.preventDefault();
		nhbParams.new_post_status = 'draft';
		this._domConfirm.click();
		document.querySelector('#nhb-save-button').setAttribute( 'data-current-post-type', 'draft' );
		this._domConfirm.querySelector('.ab-item').childNodes[1].nodeValue = nhbParams.labels.save_as_draft;
	}.bind(this));
	document.querySelector( '#wp-admin-bar-gambit_builder_save_options #nhb-save-pending' ).addEventListener( 'click', function(ev) {
		ev.preventDefault();
		nhbParams.new_post_status = 'pending';
		this._domConfirm.click();
		document.querySelector('#nhb-save-button').setAttribute( 'data-current-post-type', 'pending' );
		this._domConfirm.querySelector('.ab-item').childNodes[1].nodeValue = nhbParams.labels.save_as_pending;
	}.bind(this));

	wp.hooks.doAction( 'nhb.ct.mounted' );
...
( function() {
	...
	ContentEdit.ResizableElement.prototype._onMouseDown = function(ev) {
		...
		if ( corner ) {
			if ( window.nhbEditor.isCtrlDown && window.nhbEditor.isShiftDown ) {
...
nhbEditor.Frame = wp.media.view.Frame.extend({
	className: 'nhb-icon-modal',
	template:  wp.template( 'nhb-icon-frame' ),
	...
	open: function( args ) {
		...
		this.modal.open();
		this.modal.el.children[0].classList.add( 'nhb-modal-frame' );
		if ( this.className ) {
		...
		this.modal.el.children[0].classList.add( 'nhb-frame-hide' );
		setTimeout( function() {
			this.modal.el.children[0].classList.remove( 'nhb-frame-hide' );
		}.bind(this), 50 );
	},
...
nhbEditor.SearchFrame = nhbEditor.Frame.extend({
	className: 'nhb-icon-modal',
	template:  wp.template( 'nhb-icon-frame' ),
	...
	_onOpen: function() {
		...
		nhbEditor.Frame.prototype._onOpen.apply( this );
		setTimeout( function() {
		...
		this.modal.el.querySelector( '.nhb-no-results' ).style.display = '';
	...
	reset: function() {
		this.selected = null;
		this.model.el.querySelector( '.nhb-no-results' ).style.display = '';
		this.modal.el.querySelector( '.media-toolbar-primary button' ).setAttribute( 'disabled', 'disabled' );
	},
	doSearch: function( ev ) {
		var keyword = ev.target.value.trim().toLowerCase();
		var shortcodes = this.modal.el.querySelectorAll( '.nhb-search-list > *' );
		var hasResult = false;
		...
		this.modal.el.querySelector( '.nhb-no-results' ).style.display = hasResult ? '' : 'flex';
	},
	select: function( ev ) {
		var target = ev.target;
		while ( ! target.parentNode.classList.contains( 'nhb-search-list' ) ) {
			target = target.parentNode;
		}
		this.selected = target;
		if ( this.modal.el.querySelector( '.nhb-selected' ) ) {
			this.modal.el.querySelector( '.nhb-selected' ).classList.remove( 'nhb-selected' );
		}
		target.classList.add( 'nhb-selected' );
 ...
 *
 * Call by using: nhbEditor.iconFrame.open(). Additional arguments may be given.
 */

/* globals nhbParams, nhbEditor */

nhbEditor._IconFrame = nhbEditor.Frame.extend({
	className: 'nhb-icon-modal',
	template:  wp.template( 'nhb-icon-frame' ),
	...
	events: {
		...
		'click .nhb-icon-display [data-name]': 'selectIcon'
	},
	_onOpen: function() {
		nhbEditor.Frame.prototype._onOpen.apply( this );
		setTimeout( function() {
	...
	reset: function() {
		this._searchResults = [];
		this.modal.el.querySelector( '.nhb-no-results' ).style.display = '';
		while ( this.modal.el.querySelector( '.nhb-search-list > *:not(.nhb-no-results)' ) ) {
			var item = this.modal.el.querySelector( '.nhb-search-list > *:not(.nhb-no-results)' );
	...
	doSearch: function( ev ) {
		...
		// Remember searches.
		if ( typeof nhbParams.icon_searches === 'undefined' ) {
			nhbParams.icon_searches = [];
		}
		if ( typeof ev.keyCode !== 'undefined' ) {
			nhbParams.icon_searches.push( keyword );
		}

		var request = new XMLHttpRequest();
		request.open( 'POST', nhbParams.ajax_url, true );
		...
		request.send( 'action=nhb_icon_search&nonce=' + nhbParams.nonce + '&s=' + keyword );
	},
	displayNoResults: function() {
		this.modal.el.querySelector( '.nhb-no-results' ).style.display = 'flex';
		// var item = document.createElement( 'div' );
		// item.classList.add( 'nhb-no-results' );
		// item.innerHTML = 'No matches found';
		// this.modal.el.querySelector( '.nhb-icon-display' ).appendChild( item );
	},
	displayResults: function() {
		...
		if ( typeof this.groups === 'undefined' ) {
			this.groups = {};
			for ( groupLabel in nhbParams.icon_groups ) {
				if ( nhbParams.icon_groups.hasOwnProperty( groupLabel ) ) {
					this.groups[ groupLabel ] = new RegExp( nhbParams.icon_groups[ groupLabel ], 'i' );
				}
			}
		}
		...
		if ( this._currentGroup !== currGroup ) {
			...
			item.classList.add( 'nhb-icon-group-title' );
			this.modal.el.querySelector( '.nhb-icon-display' ).appendChild( item );
		...
		this.modal.el.querySelector( '.nhb-icon-display' ).appendChild( icon );
	...
	selectIcon: function( ev ) {
		...
		this.selected = target;
		if ( this.modal.el.querySelector( '.nhb-icon-display .nhb-selected' ) ) {
			this.modal.el.querySelector( '.nhb-icon-display .nhb-selected' ).classList.remove( 'nhb-selected' );
		}
		target.classList.add( 'nhb-selected' );
...
window.addEventListener( 'DOMContentLoaded', function() {
	nhbEditor.iconFrame = new nhbEditor._IconFrame();
});

wp.hooks.addAction( 'nhb.save.payload', function( payload ) {
	if ( typeof nhbParams.icon_searches !== 'undefined' ) {
		payload.append( 'icon_searches', nhbParams.icon_searches );
	}
} );
 ...
 * The Icon picker modal popup.
 *
 * Call by using: nhbEditor.widgetFrame.open(). Additional arguments may be given.
 */

/* globals nhbEditor */

nhbEditor._WidgetFrame = nhbEditor.SearchFrame.extend({
	className: 'nhb-widget-modal',
	template:  wp.template( 'nhb-widget-frame' ),
...
window.addEventListener( 'DOMContentLoaded', function() {
	nhbEditor.widgetFrame = new nhbEditor._WidgetFrame();
});

/**
 * The shortcode picker modal popup.
 *
 * Call by using: nhbEditor.shortcodeFrame.open(). Additional arguments may be given.
 */

/* globals nhbEditor, nhbParams, nhbInspectorOptions */

nhbEditor._ShortcodeFrame = nhbEditor.SearchFrame.extend({
	className: 'nhb-shortcode-modal',
	template:  wp.template( 'nhb-shortcode-frame' ),

	events: {
		'click .media-toolbar-primary button': '_primaryClicked',
		'keyup [type="search"]': 'searchKeyup',
		'click [data-shortcode-tag]': 'select',
		'click .nhb-refresh-mappings': 'refreshShortcodeMappings'
	},
	_onOpen: function() {
		nhbEditor.SearchFrame.prototype._onOpen.apply( this );
		this.initShortcodeList();
	},
	initShortcodeList: function() {
		var shortcodeArea = this.modal.el.querySelector( '.nhb-search-list' ), map;
		if ( ! shortcodeArea.querySelector( '*:not(.nhb-no-results)' ) ) {
			var allShortcodes = [], sc, tag;

			// Gather all shortcodes.
			for ( var i = 0; i < nhbParams.shortcodes.length; i++ ) {
				tag = nhbParams.shortcodes[ i ];

				// Allow shortcodes to be hidden from the shortcode picker.
				if ( typeof nhbParams.shortcodes_to_hide !== 'undefined' && -1 !== nhbParams.shortcodes_to_hide.indexOf( tag ) ) {
					continue;
				}
				...
				if ( nhbParams.shortcode_mappings[ tag ] ) {
					map = nhbParams.shortcode_mappings[ tag ];
					if ( map.is_hidden === '1' ) {
						continue;
					}
					...
					sc.image = 'http://ps.w.org/' + sc.ownerslug + '/assets/icon-128x128.png), url(http://ps.w.org/' + sc.ownerslug + '/assets/icon.svg), url(' + nhbParams.plugin_url + 'no_hassle_builder/assets/element-icons/shortcode_single.svg';

					// If theme shortcode, use the theme's screenshot as the icon.
					if ( map.owner === nhbParams.theme ) {
						sc.image = nhbParams.stylesheet_directory_uri + 'screenshot.png';
					}
				...
				if ( nhbInspectorOptions.Shortcode[ nhbParams.shortcodes[ i ] ] ) {
					map = nhbInspectorOptions.Shortcode[ nhbParams.shortcodes[ i ] ];
					if ( map.is_hidden === '1' ) {
						continue;
					}
					if ( map.label ) {
			...
			for ( tag in allShortcodes ) {
				if ( allShortcodes.hasOwnProperty( tag ) ) {
					...
					owner.classList.add( 'nhb-shortcode-owner' );
					owner.innerHTML = sc.owner + ' [' + sc.tag + ']';
					div.appendChild( owner );
					if ( sc.isMapped ) {
						var preview = document.createElement( 'DIV' );
						preview.classList.add( 'nhb-shortcode-owner-image' );
						if ( sc.image ) {
							preview.setAttribute( 'style', 'background-image: url(' + sc.image + ')' );
						}
						div.appendChild( preview );
						div.classList.add( 'nhb-has-owner-image' );
					}
	...
	refreshShortcodeMappings: function(ev) {
		...
		this.modal.$el.addClass( 'nhb-busy' );

	   	var payload = new FormData();
	   	payload.append( 'action', 'nhb_update_shortcode_mappings' );
	   	payload.append( 'nonce', nhbParams.nonce );

	   	var xhr = new XMLHttpRequest();

	   	xhr.onload = function() {
			this.modal.$el.removeClass( 'nhb-busy' );
	   		if ( xhr.status >= 200 && xhr.status < 400 ) {
				...
				if ( response ) {
					...
					for ( var shortcode in nhbParams.shortcode_mappings ) {
						if ( nhbParams.shortcode_mappings.hasOwnProperty( shortcode ) ) {
							window.nhbRemoveInspector( shortcode );
						}
					}

					// Add the mappings into the correct place.
					nhbParams.shortcode_mappings = response;

					// Remove all the shortcodes.
					var shortcodeArea = this.modal.el.querySelector( '.nhb-search-list' );
					var noResults = shortcodeArea.querySelector( '.nhb-no-results' );
...
window.addEventListener( 'DOMContentLoaded', function() {
	nhbEditor.shortcodeFrame = new nhbEditor._ShortcodeFrame();
});
...
nhbEditor._PredesignedFrame = nhbEditor.SearchFrame.extend({
	className: 'nhb-predesigned-modal',
	template:  wp.template( 'nhb-predesigned-frame' ),
	...
	initList: function() {
		if ( this.modal.el.querySelector( '.nhb-search-list > *:not(.nhb-no-results)' ) ) {
			return;
		}

		var container = this.modal.el.querySelector( '.nhb-search-list' );
...
window.addEventListener( 'DOMContentLoaded', function() {
	nhbEditor.predesignedFrame = new nhbEditor._PredesignedFrame();
});

/**
 * The HTML editor modal popup.
 *
 * Call by using: nhbEditor.htmlFrame.open(). Additional arguments may be given.
 */

/* globals nhbEditor */

nhbEditor._HtmlFrame = nhbEditor.Frame.extend({
	className: 'nhb-html-modal',
	template:  wp.template( 'nhb-html-frame' ),
...
window.addEventListener( 'DOMContentLoaded', function() {
	nhbEditor.htmlFrame = new nhbEditor._HtmlFrame();
});

/**
 * The admin settings modal popup.
 *
 * Call by using: nhbEditor.adminFrame.open(). Additional arguments may be given.
 */

/* globals nhbEditor */

nhbEditor._AdminFrame = nhbEditor.Frame.extend({
	className: 'nhb-admin-modal',
	template:  wp.template( 'nhb-admin-frame' ),
	...
	_onOpen: function() {
		...
		// that we are inside a nhb iframe.
		sessionStorage.setItem( 'nhb_in_admin_iframe', 1 );

		setTimeout( function() {
			this.modal.el.classList.add( 'nhb-busy' );
			var iframe = this.modal.el.querySelector( 'iframe' );
			iframe.onload = function() {
				this.modal.el.classList.remove( 'nhb-busy' );
			}.bind( this );
	...
	_onClose: function() {
		...
		// Reset the session storage value for the nhb iframe.
		sessionStorage.removeItem( 'nhb_in_admin_iframe' );

		this.modal.el.querySelector( 'iframe' ).setAttribute( 'src', '' );

		nhbEditor.Frame.prototype._onClose.apply( this );
	}
});

window.addEventListener( 'DOMContentLoaded', function() {
	nhbEditor.adminFrame = new nhbEditor._AdminFrame();
});

/* globals ContentTools, ContentEdit, nhbEditor */

nhbEditor.Overlays = [];

nhbEditor.Overlay = (function() {
	...
	function Overlay() {
		...
		nhbEditor.Overlays.push( this );

		wp.hooks.addAction( 'nhb.element.over', function( element ) {
		...
		document.addEventListener( 'mouseover', function( ev ) {
			...
			if ( ! Overlay.active ) {
				if ( window.nhbSelectorMatches( ev.target, '[data-name="main-content"], [data-name="main-content"] *, .nhb-quick-action-overlay, .nhb-quick-action-overlay *, .nhb-toolbar, .nhb-toolbar *' ) ) {
					return;
				}
				this._hide();
			}
	...
	Overlay.prototype.mount = function() {
		this._domElement.classList.add( 'nhb-quick-action-overlay' );
		this._domElement.classList.add( 'nhb-overlay-' + this.constructor.name.toLowerCase() );
		if ( this._classname ) {
	...
	Overlay.prototype._mousedown = function(ev) {
		...
		this.deltaY = 0;
		Overlay.active = true;
		this._domElement.classList.add( 'nhb-active' );
		document.body.classList.add( 'nhb-overlay-is-active' );
		document.body.classList.add( 'nhb-overlay-' + this.constructor.name.toLowerCase() );
		this.onClick( ev );
		this.onMoveStart( ev );
	};
	...
	Overlay.hideOtherOverlays = function( callingOverlay ) {
		Array.prototype.forEach.call( nhbEditor.Overlays, function( overlay ) {
	...
	Overlay.prototype._mouseup = function() {
		...
		this._domElement.classList.remove( 'nhb-active' );
		document.body.classList.remove( 'nhb-overlay-is-active' );
		document.body.classList.remove( 'nhb-overlay-active-' + this.constructor.name.toLowerCase() );
	};

	Overlay.prototype._mouseenter = function( ev ) {
		this._domElement.classList.add( 'nhb-over' );
		document.body.classList.add( 'nhb-overlay-hovered' );
		document.body.classList.add( 'nhb-overlay-hovered-' + this.constructor.name.toLowerCase() );
		this.onEnter( ev );
	};

	Overlay.prototype._mouseleave = function() {
		this._domElement.classList.remove( 'nhb-over' );
		document.body.classList.remove( 'nhb-overlay-hovered' );
		document.body.classList.remove( 'nhb-overlay-hovered-' + this.constructor.name.toLowerCase() );
		this.onLeave();
	};
...
( function() {
	...
	var nhbOverElement = _.throttle( function( target ) {
		if ( currentTarget === target ) {
			wp.hooks.doAction( 'nhb.element.over', currentOverElement );
			return;
		}
		if ( ! target._ceElement ) {
			wp.hooks.doAction( 'nhb.nonelement.over', target );
		...
		wp.hooks.doAction( 'nhb.element.over', target._ceElement );
	}, 30 );
	var mouseListener = function( ev ) {
		nhbOverElement( ev.target );
	};
...
/* globals ContentTools, ContentEdit, nhbEditor, __extends */

nhbEditor.OverlayControls = ( function( _super ) {
	...
	OverlayControls.prototype.createElement = function() {
		...
		wrapper.classList.add( 'nhb-overlay-wrapper' );
		element.appendChild( wrapper );

		for ( var i = 0; i < this.controls.length; i++ ) {
			var control = document.createElement( 'DIV' );
			control.classList.add( 'nhb-overlay-' + this.controls[ i ].name );
			control.control = this.controls[ i ];
			this.controls[ i ]._domElement = control;
			control.addEventListener( 'mouseenter', function( overlay ) {
				overlay._domElement.classList.add( 'nhb-over-' + this.control.name );
				this.control._domElement.classList.add( 'nhb-control-over' );
			}.bind( control, this ) );
			control.addEventListener( 'mouseleave', function( overlay ) {
				overlay._domElement.classList.add( 'nhb-over-' + this.control.name );
				this.control._domElement.classList.remove( 'nhb-control-over' );
			}.bind( control, this ) );
	...
	OverlayControls.prototype.updatePosition = function( element ) {
		...
		var toolbar = getComputedStyle( document.querySelector( '.nhb-toolbox-bar' ) );
	...
	OverlayControls.prototype._hide = function() {
		if ( this._domElement && this._domElement.getAttribute( 'class' ).indexOf( 'nhb-active-' ) !== -1 ) {
			return;
		}
	...
	OverlayControls.prototype.hide = function() {
		...
		this._domElement.classList.remove( 'nhb-overlay-show' );

		clearInterval( this._updatePositionTimeout );

		for ( var i = 0; i < this.controls.length; i++ ) {
			this._domElement.classList.remove( 'nhb-active-' + this.controls[ i ].name );
		}
	};
	...
	OverlayControls.prototype.show = function( element ) {
		...
		if ( this._domElement.classList.contains( 'nhb-overlay-show' ) ) {
		...
		this._domElement.classList.add( 'nhb-overlay-show' );

		for ( var i = 0; i < this.controls.length; i++ ) {
			this._domElement.classList.remove( 'nhb-active-' + this.controls[ i ].name );
			this._domElement.classList.remove( 'nhb-over-' + this.controls[ i ].name );
			this.controls[ i ]._domElement.classList.remove( 'nhb-control-active' );
			this.controls[ i ]._domElement.classList.remove( 'nhb-control-over' );
		}
	};
	...
	OverlayControls.prototype.onClick = function( ev ) {
		...
		for ( i = 0; i < this.controls.length; i++ ) {
			if ( ev.target === this.controls[ i ]._domElement ) {
				...
				this._domElement.classList.add( 'nhb-active-' + this.controls[ i ].name );
				this._domElement.classList.add( 'nhb-active' );
				this.controls[ i ]._domElement.classList.add( 'nhb-control-active' );
				break;
			}
		}
...
/* globals nhbEditor, __extends, nhbParams */

nhbEditor.OverlayElement = ( function( _super ) {
		...
		var controls = [
			{
				...
				onMoveStart: function( overlay, element, styles, rect ) {
					...
					this._overlaySize.classList.add( 'nhb-size-indicator' );
				...
				onMove: function( overlay, element, deltaX ) {
					...
					if ( window.nhbEditor.isShiftDown ) {
			...
			},
			{
				...
				refresh: function( overlay, element, styles, rect ) {
					...
					if ( rect.width > 100 ) {
						this._domElement.setAttribute( 'data-label', nhbParams.labels.margin + ': ' + styles.marginTop );
					} else {
			...
			},
			{
				...
				refresh: function( overlay, element, styles, rect ) {
					...
					if ( rect.width > 100 ) {
						this._domElement.setAttribute( 'data-label', nhbParams.labels.margin + ': ' + styles.marginBottom );
					} else {
...
} )( nhbEditor.OverlayControls );

/* globals nhbEditor, __extends, nhbParams */


nhbEditor.OverlayColumn = ( function( _super ) {
		...
		var controls = [
			{
				...
				refresh: function( overlay, element, styles ) {
					...
					this._domElement.setAttribute( 'data-label', nhbParams.labels.padding + ': ' + styles.paddingTop );
			...
			},
			{
				...
				refresh: function( overlay, element, styles ) {
					...
					this._domElement.setAttribute( 'data-label', nhbParams.labels.padding + ': ' + styles.paddingBottom );
			...
			},
			{
				...
				refresh: function( overlay, element, styles, rect ) {
					...
					if ( parseInt( rect.height, 10 ) > 100 ) {
						this._domElement.setAttribute( 'data-label', nhbParams.labels.padding + ': ' + styles.paddingLeft );
					} else {
			...
			},
			{
				...
				refresh: function( overlay, element, styles, rect ) {
					...
					if ( parseInt( rect.height, 10 ) > 100 ) {
						this._domElement.setAttribute( 'data-label', nhbParams.labels.padding + ': ' + styles.paddingRight );
					} else {
...
/* globals nhbEditor, ContentEdit, __extends, nhbParams */

nhbEditor.OverlayRow = (function(_super) {
	...
	OverlayRow.prototype.createElement = function() {
		...
		this._topMargin.classList.add( 'nhb-overlay-margin-top' );
		this._topMargin.addEventListener( 'mouseenter', function() {
			this._domElement.classList.add( 'nhb-over-margin-top' );
		}.bind( this ) );
		this._topMargin.addEventListener( 'mouseleave', function() {
			this._domElement.classList.remove( 'nhb-over-margin-top' );
		}.bind( this ) );
		element.appendChild( this._topMargin );

		this._bottomMargin = document.createElement( 'DIV' );
		this._bottomMargin.classList.add( 'nhb-overlay-margin-bottom' );
		this._bottomMargin.addEventListener( 'mouseenter', function() {
			this._domElement.classList.add( 'nhb-over-margin-bottom' );
		}.bind( this ) );
		this._bottomMargin.addEventListener( 'mouseleave', function() {
			this._domElement.classList.remove( 'nhb-over-margin-bottom' );
		}.bind( this ) );
	...
	OverlayRow.prototype.show = function( element ) {
		...
		this._domElement.classList.remove( 'nhb-active-margin-top' );
		this._domElement.classList.remove( 'nhb-active-margin-bottom' );

		this.updatePosition( element );

		this._topMargin.setAttribute( 'data-label', nhbParams.labels.margin + ': ' + styles.marginTop );
		this._topMargin.style.height = styles.marginTop;
		this._topMargin.setAttribute( 'data-value', 'margin-' + styles.marginTop );

		this._bottomMargin.setAttribute( 'data-label', nhbParams.labels.margin + ': ' + styles.marginBottom );
		...
		for ( i = 0; i < element.children.length - 1; i++ ) {
			...
			o.classList.add( 'nhb-overlay-column-width-' + i );
			o.classList.add( 'nhb-overlay-column-width' );
			o._index = i;
			o._this = this;
			if ( rect.height > 100 ) {
				o.setAttribute( 'data-label', nhbParams.labels.column_width );
			} else {
				o.setAttribute( 'data-label', nhbParams.labels.width );
			}

			o.addEventListener( 'mouseenter', function() {
				this._this._domElement.classList.add( 'nhb-over-column-width-' + this._index );
				this._this._domElement.classList.add( 'nhb-overlay-column-width' );
			} );
			o.addEventListener( 'mouseleave', function() {
				this._this._domElement.classList.remove( 'nhb-over-column-width-' + this._index );
				this._this._domElement.classList.remove( 'nhb-overlay-column-width' );
			} );
		...
		for ( i = 0; i < element.children.length; i++ ) {
			...
			var label = document.createElement( 'DIV' );
			label.classList.add( 'nhb-overlay-column-label' );
	...
	OverlayRow.prototype.onMove = function() {
		...
		if ( this._locationDragging === 'margin-top' ) {
			...
			this._topMargin.setAttribute( 'data-label', nhbParams.labels.margin + ': ' + margin + 'px' );
			...
			// var rect = this.element._domElement.getBoundingClientRect();
			if ( rect.width > 100 ) {
				this._bottomMargin.setAttribute( 'data-label', nhbParams.labels.margin + ': ' + margin + 'px' );
			} else {
			...
			this.element._domElement.classList.add( 'nhb-overlay-changing-cols' );
			for ( i = 0; i < this.element.children.length; i++ ) {
		...
		for ( i = 0; i < this.element.children.length; i++ ) {
			...
			this._columnLabels[ i ].classList.add( 'nhb-overlay-column-label' );
	...
	OverlayRow.prototype._mouseup = function() {
		...
		if ( this.element && this.element._domElement ) {
			this.element._domElement.classList.remove( 'nhb-overlay-changing-cols' );
		}
		this._domElement.classList.remove( 'nhb-active-overlay-column' );
	};
	...
	OverlayRow.prototype.onClick = function( ev ) {
		...
		if ( ev.target === this._topMargin ) {
			...
			for ( i = 0; i < this._columnWidths.length; i++ ) {
				if ( ev.target === this._columnWidths[ i ] ) {
					...
					this._domElement.classList.add( 'nhb-active-overlay-column' );
					break;
				}
			}
		}
		...
		this._domElement.classList.remove( 'nhb-active-margin-bottom' );
		for ( i = 0; i < 10; i++ ) {
			this._domElement.classList.remove( 'nhb-active-column-' + i );
		}
		if ( this._locationDragging ) {
			this._domElement.classList.add( 'nhb-active-' + this._locationDragging );
		}
	};
...
/* globals ContentEdit, nhbParams, ContentTools */
...
( function() {
	...
	var showDropIndicator = function( placement ) {
		...
		if ( ! dropIndicator ) {
			...
			dropIndicator.classList.add( 'nhb-drop-indicator' );
		...
		dropIndicator.firstChild.innerHTML = nhbParams.labels.move_above_s.replace(  /%s/, root._dropTarget.typeName() );
		dropIndicator.firstChild.nextSibling.innerHTML = nhbParams.labels.move_below_s.replace(  /%s/, root._dropTarget.typeName() );
		...
		dropIndicator.classList.remove( 'nhb-drop-indicator-small' );
		if ( rect.height + parseInt( styles.marginTop, 10 ) + parseInt( styles.marginBottom, 10 ) < 60 ) {
			dropIndicator.classList.add( 'nhb-drop-indicator-small' );
		}

		// Add the classes for the drop indicator (whether to show up or down).
		dropIndicator.classList.remove( 'nhb-drop-indicator-above' );
		dropIndicator.classList.remove( 'nhb-drop-indicator-below' );
		dropIndicator.classList.add( 'nhb-drop-indicator-' + placement[0] );
		...
		if ( dropTargetDom.tagName === 'P' && dropTargetDom.parentNode.tagName === 'DIV' ) {
			if ( dropTargetDom.parentNode.classList && dropTargetDom.classList ) {
				if ( dropTargetDom.parentNode.classList.contains( 'nhb-col' ) && dropTargetDom.classList.contains( 'ce-element--empty' ) ) {
					dropIndicator.firstChild.innerHTML = nhbParams.labels.move_inside_column;
					dropIndicator.firstChild.nextSibling.innerHTML = nhbParams.labels.move_inside_column;
					dropIndicator.classList.remove( 'nhb-drop-indicator-above' );
					dropIndicator.classList.add( 'nhb-drop-indicator-below' );
				}
			}
		}
		...
		setTimeout( function() {
			if ( dropIndicator ) {
				dropIndicator.classList.add( 'nhb-drop-indicator-show' );
			}
		}, 10 );
	...
	_Root.startDragging = function( element, x, y ) {
		...
		setTimeout( function() {
			if ( this._draggingDOMElement ) {
				this._draggingDOMElement.classList.add( 'nhb-drag-helper-show' );
			}
		}.bind( this ), 10 );
		return ret;
	};
	 ...
	 */
	var nhbOverElement = _.throttle( function( ev ) {
		...
		if ( window.nhbSelectorMatches( ev.target, '.ct-widget, .ct-widget *' ) ) {
			hideDropIndicator();
			root._dropTarget = null;
			return;
		}

		var elements = document.querySelectorAll( '.nhb-main-wrapper > .ce-element, .nhb-main-wrapper > [data-ce-tag]' );
		...
		Array.prototype.forEach.call( elements, function( el ) {
			...
			// window.
			if ( window.nhbSelectorMatches( el, '.ce-element *, [data-ce-tag] *' ) ) {
				doExit = true;
				return;
			}
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	for ( var i = 0; i < findInThese.length; i++ ) {
		if ( document.querySelectorAll( findInThese[ i ] + ' [data-nhb-title-marker-post-id]' ).length ) {
			titleMarkers = document.querySelectorAll( findInThese[ i ] + ' [data-nhb-title-marker-post-id]' );
		}
	}

	if ( ! titleMarkers ) {
		titleMarkers = document.querySelectorAll( '[data-nhb-title-marker-post-id]' );
	}
	...
	Array.prototype.forEach.call( titleMarkers, function( marker ) {
		...
		var postID = marker.getAttribute( 'data-nhb-title-marker-post-id' );

		titleElement.classList.add( 'nhb-title-editor' );
		titleElement.getAttribute( 'nhb-post-id', postID );
	...
	nhbEditor.Title = ( function() {
		...
		Title.prototype._onFocus = function() {
			...
			nhbEditor.Overlay.hideOtherOverlays( null );
		};
		...
		Title.prototype._onMouseEnter = function() {
			wp.hooks.doAction( 'nhb.element.over', this );
		};
	...
	for ( i = 0; i < titles.length; i++ ) {
		titles[ i ] = new nhbEditor.Title( titles[ i ] );
	}
...
/* globals nhbEditor, __extends */

nhbEditor.Toolbar = ( function( _super ) {
	...
	Toolbar.prototype.createElement = function() {
		var element = document.createElement( 'DIV' );
		element.classList.add( 'nhb-overlay-toolbar' );
		element.classList.add( 'nhb-toolbar-' + this.elementName.toLowerCase().replace( /[^\w\d]/, '-' ) );

		var buttonContainer = document.createElement( 'DIV' );
		buttonContainer.classList.add( 'nhb-toolbar-wrapper' );
		...
		for ( var i = 0; i < this.tools.length; i++ ) {
			...
			tool.classList.add( 'nhb-toolbar-tool' );

			if ( this.tools[ i ].label ) {
				tool.classList.add( 'nhb-toolbar-label' );
				tool.innerHTML = this.tools[ i ].label;
				buttonContainer.appendChild( tool );
				continue;
			}

			tool.classList.add( 'nhb-toolbar-tool-' + this.tools[ i ].id );
	...
	Toolbar.prototype.canApply = function( element ) {
		...
		if ( element.constructor.name === this.elementName ) {
			...
			for ( var i = 0; i < this.tools.length; i++ ) {
				...
				this._domElement.children[0].children[ i ].classList.remove( 'nhb-toolbar-tool-hide' );
				if ( typeof this.tools[ i ].display === 'function' ) {
					if ( ! this.tools[ i ].display( element ) ) {
						this._domElement.children[0].children[ i ].classList.add( 'nhb-toolbar-tool-hide' );
					}
				}
			}
	...
	Toolbar.prototype._updateFirstLastButtons = function() {
		var button = this._buttonContainer.querySelector( '.nhb-toolbar-button-first' );
		if ( button ) {
			button.classList.remove( 'nhb-toolbar-button-first' );
		}
		button = this._buttonContainer.querySelector( '.nhb-toolbar-button-last' );
		if ( button ) {
			button.classList.remove( 'nhb-toolbar-button-last' );
		}

		for ( var i = 0; i < this._buttonContainer.children.length; i++ ) {
			if ( this._buttonContainer.children[ i ].offsetWidth !== 0 ) {
				this._buttonContainer.children[ i ].classList.add( 'nhb-toolbar-button-first' );
				break;
			}
		}
		for ( i = this._buttonContainer.children.length - 1; i >= 0; i-- ) {
			if ( this._buttonContainer.children[ i ].offsetWidth !== 0 ) {
				this._buttonContainer.children[ i ].classList.add( 'nhb-toolbar-button-last' );
				break;
			}
		}
	};
...
/* globals ContentTools, nhbEditor, __extends, nhbParams */

nhbEditor.ToolbarElement = ( function( _super ) {
	...
	toolbars.push(
		{
			id: 'remove',
			tooltip: nhbParams.labels['delete'],
...
/* globals nhbEditor, __extends, ContentTools, nhbParams */

nhbEditor.ToolbarImage = ( function( _super ) {
	...
	toolbars.push(
		{
			id: 'align-left',
			tooltip: nhbParams.labels.align_left,
			onClick: function( element ) {
		...
		{
			id: 'align-center',
			tooltip: nhbParams.labels.align_center,
			onClick: function( element ) {
				element.removeCSSClass( 'alignleft' );
				element.removeCSSClass( 'alignright' );
				element.removeCSSClass( 'aligncenter' );
		...
		{
			id: 'align-right',
			tooltip: nhbParams.labels.align_right,
			onClick: function( element ) {
				element.removeCSSClass( 'alignleft' );
				element.removeCSSClass( 'alignright' );
				element.removeCSSClass( 'aligncenter' );
...
/* globals nhbEditor, ContentTools, __extends, nhbParams */

nhbEditor.ToolbarNewsletter = ( function( _super ) {
	...
	function ToolbarNewsletter() {
		...
		ToolbarNewsletter.__super__.constructor.call( this,
			...
			[
				{
					label: nhbParams.labels.newsletter
				},
				{
					id: 'move',
					tooltip: nhbParams.labels.move,
				...
				{
					id: 'settings',
					tooltip: nhbParams.labels.properties,
					onClick: function( element ) {
...
nhbEditor.ToolbarHtml = ( function( _super ) {
...
nhbEditor.ToolbarIframe = ( function( _super ) {
...
nhbEditor.ToolbarEmbed = ( function( _super ) {
	...
	function ToolbarEmbed() {
		...
		ToolbarEmbed.__super__.constructor.call( this,
			...
			[
				{
					label: nhbParams.labels.embedded_url
				},
				{
					id: 'edit',
					tooltip: nhbParams.labels.edit,
...
nhbEditor.ToolbarMap = ( function( _super ) {
...
nhbEditor.ToolbarIcon = ( function( _super ) {
	...
	function ToolbarIcon() {
		...
		ToolbarIcon.__super__.constructor.call( this,
			...
			[
				...
				{
					id: 'clone',
					tooltip: nhbParams.labels.clone,
					onClick: function( element ) {
						element.clone();
					}
				},
	...
	ToolbarIcon.prototype.canApply = function( element ) {
		...
		if ( ret ) {
			this._domElement.classList.remove( 'nhb-small-toolbar' );
			var rect = this._domElement.getBoundingClientRect();
			if ( rect.width < 130 ) {
				this._domElement.classList.add( 'nhb-small-toolbar' );
			}
		}
		return ret;
	};
...
/* globals nhbEditor, nhbInspectorOptions, __extends, ContentTools, nhbParams */

nhbEditor.ToolbarShortcode = ( function( _super ) {
			...
			[
				{
					label: function( element ) {
						var label = wp.hooks.applyFilters( 'nhb.toolbar.shortcode.label', element.sc_base );

						// Use the label of the shortcode if it is provided.
						if ( nhbInspectorOptions.Shortcode[ element.sc_base ] ) {
							if ( nhbInspectorOptions.Shortcode[ element.sc_base ].label ) {
								label = wp.hooks.applyFilters( 'nhb.toolbar.shortcode.label', nhbInspectorOptions.Shortcode[ element.sc_base ].label );
							}
						}
...
nhbEditor.ToolbarList = ( function( _super ) {
...
/* globals nhbEditor, __extends, ContentTools, nhbParams, ContentEdit */

nhbEditor.ToolbarRow = ( function( _super ) {
		...
		ToolbarRow.__super__.constructor.call( this,
			...
			[
				 ...
				 */
				{
					...
					label: function() {
						return nhbParams.labels.carousel;
					}
				},
				{
					id: 'move',
					tooltip: nhbParams.labels.move_carousel,
					'class': 'nhb-toolbar-yellow',
					display: function( element ) {
						// Only show when inside a carousel.
						return element.parent()._domElement.classList.contains( 'glide__slide' );
				...
				{
					id: 'minus',
					tooltip: nhbParams.labels.remove_slide,
					'class': 'nhb-toolbar-yellow',
					display: function( element ) {
				...
				{
					id: 'add',
					tooltip: nhbParams.labels.add_slide,
					'class': 'nhb-toolbar-yellow',
					display: function( element ) {
						// Only show when inside a carousel.
						return element.parent()._domElement.classList.contains( 'glide__slide' );
				...
				{
					id: 'clone',
					tooltip: nhbParams.labels.clone_slide,
					'class': 'nhb-toolbar-yellow',
					display: function( element ) {
						// Only show when inside a carousel.
						return element.parent()._domElement.classList.contains( 'glide__slide' );
				...
				{
					id: 'remove',
					tooltip: nhbParams.labels.delete_carousel,
					'class': 'nhb-toolbar-yellow',
					display: function( element ) {
						// Only show when inside a carousel.
						return element.parent()._domElement.classList.contains( 'glide__slide' );
				...
				{
					id: 'add',
					tooltip: nhbParams.labels.add_column,
					onClick: function( element ) {
				...
				{
					id: 'clone',
					tooltip: nhbParams.labels.clone_row,
					display: function( element ) {
						// Hide when the row is inside a carousel.
						if ( element.parent()._domElement.classList.contains( 'glide__slide' ) ) {
							return false;
					...
					onClick: function( element ) {
						...
						window._nhbFixRowWidth( newRow._domElement );
					}
				},
				{
					id: 'remove',
...
nhbEditor.ToolbarColumn = ( function( _super ) {
	...
	ToolbarColumn.prototype.checkToolbarSize = function( element ) {
		...
		var colToolbar = document.querySelector( '.nhb-toolbar-divcol.nhb-quick-action-overlay .nhb-toolbar-wrapper' );
		var rowToolbar = document.querySelector( '.nhb-toolbar-divrow.nhb-quick-action-overlay .nhb-toolbar-wrapper' );

		this._domElement.classList.remove( 'nhb-small-toolbar' );
		rowToolbar.parentNode.classList.remove( 'nhb-small-toolbar' );
		this._domElement.classList.remove( 'nhb-smaller-toolbar' );
		rowToolbar.parentNode.classList.remove( 'nhb-smaller-toolbar' );
		this._domElement.classList.remove( 'nhb-tiny-toolbar' );
		rowToolbar.parentNode.classList.remove( 'nhb-tiny-toolbar' );
		...
		if ( colRect.left - rowRect.right < 0 ) {
			colToolbar.parentNode.classList.add( 'nhb-small-toolbar' );
			rowToolbar.parentNode.classList.add( 'nhb-small-toolbar' );
		} else {
			return ret;
		}
		...
		if ( colRect.left - rowRect.right < 0 ) {
			colToolbar.parentNode.classList.remove( 'nhb-small-toolbar' );
			rowToolbar.parentNode.classList.remove( 'nhb-small-toolbar' );
			colToolbar.parentNode.classList.add( 'nhb-smaller-toolbar' );
			rowToolbar.parentNode.classList.add( 'nhb-smaller-toolbar' );
		} else {
			return ret;
		}
		...
		if ( colRect.left - rowRect.right < 0 ) {
			colToolbar.parentNode.classList.remove( 'nhb-smaller-toolbar' );
			rowToolbar.parentNode.classList.remove( 'nhb-smaller-toolbar' );
			colToolbar.parentNode.classList.add( 'nhb-tiny-toolbar' );
			rowToolbar.parentNode.classList.add( 'nhb-tiny-toolbar' );
		}
...
nhbEditor.ToolbarTabContainer = ( function( _super ) {
	...
	toolbar.push(
		...
		{
			id: 'add',
			tooltip: nhbParams.labels.add_tab,
			onClick: function( element ) {
...
nhbEditor.ToolbarTable = ( function( _super ) {
...
nhbEditor.ToolbarTitle = ( function( _super ) {
	...
	function ToolbarTitle() {
		...
		ToolbarTitle.__super__.constructor.call( this,
			...
			[
				{
					label: 'Post Title', //nhbParams.labels.title
				}
			]
		);
	}
...
/* globals ContentTools, nhbEditor, nhbSelectorMatches */

/**
 * The Tooltip API
 */
nhbEditor.Tooltip = ( function() {
	...
	Tooltip.prototype.init = function() {
		...
		ContentTools.EditorApp.get().bind('start', function () {
			...
			this._domElement.classList.add( 'nhb-tooltip--' + this.name );
			this._domElement.classList.add( 'nhb-tooltip' );
			...
			for ( var i = 0; i < this.tools.length; i++ ) {
				...
				tool.classList.add( 'nhb-tooltip-button' );
				tool.classList.add( 'nhb-tooltip-button-' + this.tools[ i ].name );
	...
	Tooltip.prototype._onMouseOver = function( ev ) {
		...
		if ( nhbSelectorMatches( ev.target, '.nhb-main-wrapper ' + this.selector + ', .nhb-main-wrapper ' + this.selector + ' *' ) ) {
			var target = ev.target;

			// Find the element that matched (if a child matched).
			while ( ! nhbSelectorMatches( target, '.nhb-main-wrapper ' + this.selector ) ) {
				target = target.parentNode;
			}
	...
	Tooltip.prototype._onMouseOut = function( ev ) {
		...
		// nhb-button -> tooltip / tooltip-button
		if ( nhbSelectorMatches( ev.target, '.nhb-main-wrapper ' + this.selector ) || nhbSelectorMatches( ev.target, '.nhb-main-wrapper ' + this.selector + ' *' ) ) {
			if ( ev.relatedTarget === this._domElement || this._domElement.contains( ev.relatedTarget ) ) {
				return;
			}
		}
		// tooltip / tooltip-button -> nhb-button
		if ( this._domElement === ev.target || this._domElement.contains( ev.target ) ) {
			if ( nhbSelectorMatches( ev.relatedTarget, '.nhb-main-wrapper ' + this.selector ) || nhbSelectorMatches( ev.relatedTarget, '.nhb-main-wrapper ' + this.selector + ' *' ) ) {
				return;
			}
		}
	...
	Tooltip.prototype.show = function( domElementTarget ) {
		...
		this._domElement.classList.add( 'nhb-tooltip-show' );
		this.element = domElementTarget;
	};

	Tooltip.prototype.hide = function() {
		this._domElement.classList.remove( 'nhb-tooltip-show' );
	};
...
/* globals nhbEditor, ContentSelect, ContentTools, __extends */

nhbEditor.TooltipLink = ( function( _super ) {
	__extends( TooltipLink, _super );

    function TooltipLink() {
		TooltipLink.__super__.constructor.call( this,
		 	'link',
			'.ce-element--type-text a:not(.nhb-button)',
			function( element ) {
			...
			[
				...
				},
				{
					...
					onClick: function() {
						...
						window.nhbEditor.getToolUI( 'link' ).apply( domElement._ceElement, selection, function() { } );
...
nhbEditor.TooltipButton = ( function( _super ) {
...
nhbEditor.TooltipInput = ( function( _super ) {
...
nhbEditor.TooltipTab = ( function( _super ) {
...
/* globals ContentEdit, ContentSelect, nhbEditor */
 ...
 */
nhbEditor.isNewStaticLikeElement = function( element ) {
	if ( element.constructor.name === 'Static' ) {
		return false;
	}
...
ContentEdit.Text.prototype._keyLeft = function(ev) {
	...
	if ( selection.get()[0] === 0 ) {
		...
		if ( elem ) {
			if ( nhbEditor.isNewStaticLikeElement( elem ) ) {
				ev.preventDefault();
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	document.addEventListener('keydown', function(ev) {
		...
		if ( ! nhbEditor.isNewStaticLikeElement( editor.focused() ) ) {
			return;
		}
...
ContentEdit.Node.prototype.previousContent = function() {
    var node = this.previousWithTest(function(node) {
		return node.content !== void 0 || nhbEditor.isNewStaticLikeElement( node );
    });
	return node;
};
...
/* globals ContentEdit, __extends, nhbEditor */
...
ContentEdit.StaticEditable = (function(_super) {
	...
	StaticEditable.droppers = nhbEditor.allDroppers;
...
(function() {
	...
	var mouseDownHandler = function( ev ) {
		if ( ! ev.target._ceElement ) {
			if ( window.nhbSelectorMatches( ev.target, '[data-editable]' ) ) {
				var root = ContentEdit.Root.get();
				var element = root.focused();
				if ( ! element ) {
					return;
...
/* globals ContentEdit, HTMLString, nhbParams, __extends, nhbEditor */
...
ContentEdit.Shortcode = (function(_super) {
	...
	Shortcode.prototype.mount = function() {
		...
		if ( scStyles.height === '0px' ) {
			this._domElement.classList.add('nhb--blank');
		} else {
			this._domElement.classList.remove('nhb--blank');
		}
	...
	Shortcode.fromDOMElement = function( domElement ) {
		...
		if ( nhbEditor._justAddedDesignElement ) {
	...
	Shortcode.prototype._onDoubleClick = function() {
		if ( ! wp.hooks.applyFilters( 'nhb.shortcode.allow_raw_edit', true, this.sc_base, this ) ) {
			return;
		}
		this.convertToText();
	};
		...
		if ( this._scriptsToRun ) {
			for ( var i = 0; i < this._scriptsToRun.length; i++ ) {
				try {
					...
					console.log( 'nhb:', this.sc_base, 'init code errored out.' );
				}
			}
		}

		// Run shortcode mapping init code.
		if ( typeof nhbParams.shortcode_mappings[ this.sc_base ] !== 'undefined' ) {
			var map = nhbParams.shortcode_mappings[ this.sc_base ];
	...
	Shortcode.prototype._ajaxUpdate = function( forceUpdate ) {
		...
		var payload = new FormData();
		payload.append( 'action', 'nhb_shortcode_render' );
		payload.append( 'shortcode', this.sc_hash );
		payload.append( 'nonce', nhbParams.nonce );

		this._domElement.classList.add('nhb--rendering');
		...
		request.onload = function() {
			if (request.status >= 200 && request.status < 400) {
				...
				try {
					...
					console.log( 'nhb: Error getting rendered shortcode' );

					_this._domElement.classList.remove('nhb--rendering');
				...
				document.dispatchEvent( new CustomEvent( 'nhb:shortcode-render', { detail: document.querySelector('.nhb--rendering') } ) );
			}
			_this._domElement.classList.remove('nhb--rendering');
			_this._domElement.classList.remove('nhb--blank');
			...
			if ( scStyles.height === '0px' ) {
				_this._domElement.classList.add('nhb--blank');
			} else {
				_this._domElement.classList.remove('nhb--blank');
			}
...
ContentEdit.Text.prototype.convertShortcodes = function() {
	...
	while ( shortcodeMatch ) {
		...
		if ( nhbParams.shortcodes.indexOf( shortcodeMatch[1] ) === -1 ) {
			...
			if ( elem.constructor.name === 'Shortcode' ) {
				...
				if ( i === 1 ) {
					if ( elem.sc_raw === this.origShortcode ) {
						...
						if ( scStyles.height === '0px' ) {
							elem._domElement.classList.add('nhb--blank');
						} else {
							elem._domElement.classList.remove('nhb--blank');
						}
...
ContentEdit.Hr = (function(_super) {
    ...
    Hr.droppers = nhbEditor.allDroppers;
...
/* globals ContentEdit, ContentTools, nhbParams, __extends */
...
ContentEdit.Embed = (function(_super) {
    ...
    Embed.prototype.mount = function() {
		...
		wp.hooks.doAction( 'nhb.embed.mount', this );
	...
	Embed.updateEmbedContent = function( url, textElement ) {
		...
		// var _this = this;
		var request = new XMLHttpRequest();
		request.open('POST', nhbParams.ajax_url );
		...
		request.onload = function() {
			if (request.status >= 200 && request.status < 400) {
				...
				elem._domElement.classList.remove('nhb--rendering');

				wp.hooks.doAction( 'nhb.embed.update_embed_content', elem );
...
window.addEventListener( 'DOMContentLoaded', function() {
    var editor = ContentTools.EditorApp.get();
	editor.bind( 'stop', function () {
		if ( window.nhbInitAllPretext ) {
			window.nhbInitAllPretext();
		}
	} );
} );
...
(function() {
	...
	wp.hooks.addAction( 'nhb.embed.mount', fitVideos );
	wp.hooks.addAction( 'nhb.embed.update_embed_content', fitVideos );
...
// Called when a Twitter embed element is mounted.
var nhbTwitterMount = function( element ) {
...
wp.hooks.addAction( 'nhb.embed.mount', nhbTwitterMount );
...
/* globals ContentEdit, ContentSelect, nhbSelectorMatches, __extends, nhbEditor, ContentTools, HTMLString */
...
ContentEdit.Div = (function(_super) {
	...
	Div.droppers = nhbEditor.allDroppers;
	...
	Div._fromDOMElement = function(domElement) {
		...
		} else if ( domElement.classList.contains( 'nhb-row' ) ) {
			cls = tagNames.match( 'row' );
		} else if ( domElement.classList.contains( 'nhb-col' ) ) {
...
ContentEdit.DivRow = (function(_super) {
	...
	DivRow.prototype.showOutline = function() {
		...
		while ( currElem ) {
			if ( currElem.constructor.name === 'DivRow' ) {
				currElem._addCSSClass( 'nhb-new-column' );
			}
	...
	DivRow.prototype.hideOutline = function() {
		...
		while ( currElem ) {
			if ( currElem.constructor.name === 'DivRow' ) {
				currElem._removeCSSClass( 'nhb-new-column' );
			}
			currElem = currElem.parent();
		}
	};
	...
	DivRow.prototype._onMouseOver = function(ev) {
		...
		this._removeCSSClass( 'nhb-new-column' );
		...
		if ( root._dropTarget !== null ) {
			if ( dragging && nhbSelectorMatches( root._dropTarget.domElement(), '.ce-element--dragging *' ) ) {
		...
		if ( ! ev.target.classList.contains('nhb-row') ) {
	...
	DivRow.prototype._onMouseMove = function(ev) {
		...
		if ( root._dropTarget !== null ) {
			...
			// Don't allow rows to be dragged inside themselves
			if ( nhbSelectorMatches( root._dropTarget._domElement, '.ce-element--dragging *' ) ) {
				root._dropTarget._removeCSSClass('ce-element--drop');
		        root._dropTarget._removeCSSClass('ce-element--drop-above');
		        root._dropTarget._removeCSSClass('ce-element--drop-below');
		        root._dropTarget._removeCSSClass('ce-element--drop-center');
	...
	DivRow.droppers = nhbEditor.allDroppers;

	// Make sure all rows have the nhb-row class
	DivRow.prototype.mount = function() {
		DivRow.__super__.mount.call(this);
		this.addCSSClass('nhb-row');

		// Generate a unique class if there isn't one yet.
		// var currentClass = this._domElement.getAttribute( 'class' );
		// if ( ! currentClass.match( /nhb_row_uid_/ ) ) {
		// 	this.addCSSClass( 'nhb_row_uid_' + Math.floor((1 + Math.random()) * 0x10000000).toString(36) );
		// }

		// Full-width rows get busted when mounting, this fixes them.
		if ( window._nhbFixRowWidth ) {
			window._nhbFixRowWidth( this._domElement );
		}
	};
	...
	DivRow.fromDOMElement = function(domElement) {
		...
		for (_i = 0, _len = childNodes.length; _i < _len; _i++) {
			...
			if ( childNode.tagName.toLowerCase() !== 'div' && ! childNode.classList.contains( 'nhb-col' ) ) {
				continue;
			}
...
ContentEdit.DivCol = (function(_super) {
    ...
    DivCol.prototype._onMouseDown = function(ev) {
		...
		// @see _onMouseMove
		// if ( this._domElement.classList.contains('nhb-drag-row') ) {
	...
	DivCol._dropColumn = function(element, target, placement) {
		...
		if ( target._domElement.classList.contains('nhb-drop-outside-row') ) {
	...
	*/
	...
	DivCol.prototype.mount = function() {
		...
		// Make sure columns have a .nhb-col class
		this.addCSSClass('nhb-col');

		// Generate a unique class if there isn't one yet.
		// var currentClass = this._domElement.getAttribute( 'class' );
		// if ( ! currentClass.match( /nhb_col_uid_/ ) ) {
		// 	this.addCSSClass( 'nhb_col_uid_' + Math.floor((1 + Math.random()) * 0x10000000).toString(36) );
		// }
...
// 			if ( nhbSelectorMatches( ev.target, '.ce-element--dragging *' ) ) {
...
wp.hooks.addFilter( 'nhb.save', function( html ) {

	html = html.replace(/(<[^>]+nhb-row[^>]+style=[^>]*[^-])(max-width:\s?[-0-9.\w]+;?\s?)([^>]+>)/g, '$1$3');
	html = html.replace(/(<[^>]+nhb-row[^>]+style\=[^>]*[^-])(width:\s?[-0-9.\w]+;?\s?)([^>]+>)/g, '$1$3');
	html = html.replace(/(<[^>]+nhb-row[^>]+style=[^>]*[^-])(left:\s?[-0-9.\w]+;?\s?)([^>]+>)/g, '$1$3');
	...
	html = html.replace( /<[^>]+nhb-row[^>]+data-width\=["']full-width-retain-content["'][^>]+>/g,
		function ( match ) {
	...
	html = html.replace( /<[^>]+nhb-row[^>]+data-width\=["']full-width["'][^>]+>/g,
		function ( match ) {
			match = match.replace( /(margin:\s?)([\d\w.]+)\s([\d\w.]+)\s([\d\w.]+)\s([\d\w.]+)[;"']/, 'margin-top: $2; margin-bottom: $4;' );
			match = match.replace( /(margin:\s?)([\d\w.]+)\s([\d\w.]+)\s([\d\w.]+)[;"']/, 'margin-top: $2; margin-bottom: $4;' );
			match = match.replace( /(margin:\s?)([\d\w.]+)\s([\d\w.]+)[;"']/, 'margin-top: $2; margin-bottom: $2;' );
...
/* globals ContentEdit, __extends, nhbEditor, nhbParams */
...
ContentEdit.Icon = ( function( _super ) {
    ...
    Icon.droppers = nhbEditor.allDroppers;
	...
	Icon.prototype._onDoubleClick = function() {
		nhbEditor.iconFrame.open({
			title: nhbParams.labels.icon_frame_change_title,
			button: nhbParams.labels.icon_frame_change_button,
...
ContentEdit.Html = (function(_super) {
	...
	Html.prototype.openEditor = function() {
		nhbEditor.htmlFrame.open({
			title: nhbParams.labels.html,
			button: nhbParams.labels.html_frame_button,
			successCallback: function( frameView ) {
...
/* globals nhbParams */
 ...
 // wp.hooks.addFilter( 'nhb.shortcode.allow_raw_edit', function( allow, scBase ) {
 //  if ( scBase === 'nhb_widget' ) {
 // 	 return false;
 //  }
 //  return allow;
 // } );

// wp.hooks.addFilter( 'nhb.toolbar.tools.allow', function( allow, target ) {
// 	if ( target.getAttribute('data-base') === 'nhb_widget' ) {
// 		return false;
// 	}
// 	return allow;
// } );

wp.hooks.addFilter( 'nhb.toolbar.shortcode.label', function( scBase ) {
	if ( scBase === 'nhb_sidebar' ) {
		return nhbParams.labels.sidebar;
	}
	return scBase;
} );

/* globals ContentEdit, __extends, nhbEditor, ContentTools, google */
...
ContentEdit.Map = (function(_super) {
    ...
    Map.prototype._onMouseUp = function( ev ) {
		...
		this._removeCSSClass( 'nhb-map-editing' );
    ...
    Map.prototype._onMouseMove = function( ev ) {
		...
		if ( ! this._dragging && this._clicked ) {
			this._addCSSClass( 'nhb-map-editing' );
		}
    ...
    Map.droppers = nhbEditor.allDroppers;
	...
	Map.prototype.mount = function() {
		...
		window.initnhbMaps( this._domElement, function() {
...
window.addEventListener( 'DOMContentLoaded', function() {
	var editor = ContentTools.EditorApp.get();
	if ( window.initnhbMaps ) {

		// When we end editing, the DOM gets rebuilt, we need to re-init the maps.
		editor.bind( 'stop', window.initnhbMaps );
	}

	var mapRefreshInterval;
	if ( window.nhbMapsReCenter ) {
		editor.bind( 'start', function() {
			mapRefreshInterval = setInterval( function() {
				if ( document.querySelector( '.ce-element--dragging' ) || document.querySelector( '.nhb-map-editing' ) ) {
					return;
				}
				window.nhbMapsReCenter();
			}, 1000 );
		} );
		editor.bind( 'stop', function() {
...
/* globals ContentEdit, __extends, nhbEditor, ContentSelect */
...
ContentEdit.Tabs = ( function( _super ) {
	...
	Tabs.prototype.attachTabAndContent = function( tab, index, row ) {
		var tabsID = this._domElement.getAttribute( 'class' ).match( /nhb-tabs-(\w+)/ )[0];

		var tabIndex = this.numTabs() + 1;

		var hash = window.nhbEditor.generateHash();
		while ( document.querySelector( '[id="nhb-tab-' + hash + '"]') ) {
			hash = window.nhbEditor.generateHash();
		}

		var radio = document.createElement( 'input' );
		radio.classList.add( 'nhb-tab-state' );
		radio.setAttribute( 'type', 'radio' );
		radio.setAttribute( 'name', tabsID );
		radio.setAttribute( 'id', 'nhb-tab-' + hash );
		...
		tab.attr( 'for', 'nhb-tab-' + hash );
		row.attr( 'data-panel', tabIndex );

		this._domElement.querySelector( '.nhb-tab-tabs' )._ceElement.attach( tab, index );
		this._domElement.querySelector( '.nhb-tab-panels' )._ceElement.attach( row );
	...
	Tabs.prototype.reIndexTabs = function() {
		var numTabs = this._domElement.querySelectorAll( '.nhb-tab-state' ).length;
	...
	Tabs.prototype.getOpenTab = function() {
		var radio = this._domElement.querySelector( '.nhb-tab-state:checked' )._ceElement;
	...
	Tabs.prototype.numTabs = function() {
		return this._domElement.querySelectorAll( '.nhb-tab-state' ).length;
	};

	Tabs.prototype.addTab = function() {
		var firstTab = this._domElement.querySelector( '.nhb-tab-state' )._ceElement;
		var radioID = 'nhb-tab-' + window.nhbEditor.generateHash();
		...
		var tabContainer = this._domElement.querySelector( '.nhb-tab-tabs' )._ceElement;
		...
		var panelContainer = this._domElement.querySelector( '.nhb-tab-panels' )._ceElement;
	...
	Tabs.droppers = nhbEditor.allDroppers;
...
ContentEdit.Tab = ( function( _super ) {
	...
	Tab.prototype.setActiveTab = function() {
		var activeTab = this.parentTab()._domElement.querySelector( '.nhb-tab-tabs .nhb-tab-active' );
		if ( activeTab ) {
			activeTab._ceElement.removeCSSClass( 'nhb-tab-active' );
		}
		this.addCSSClass( 'nhb-tab-active' );
	};
	...
	Tab.prototype.html = function() {
		...
		if ( this.parent().children.indexOf( this ) === 0 ) {
			if ( ! ret.match( /nhb-tab-active/ ) ) {
				var r = new RegExp( '(<' + this._tagName + '[^>]+class=[\'"])' );
				if ( ret.match( r ) ) {
					ret = ret.replace( r, '$1nhb-tab-active ' );
				} else {
					r = new RegExp( '(<' + this._tagName + ')' );
					ret = ret.replace( r, '$1 class="nhb-tab-active"' );
				}
			}
		} else {
			ret = ret.replace( /\s*nhb-tab-active\s*/g, '' );
...
wp.hooks.addFilter( 'nhb.overlay.margin_top.can_apply', function( apply, element ) {
	if ( element && element._domElement && element._domElement.parentNode && element._domElement.parentNode.classList ) {
		...
		if ( parent.classList && parent.classList.contains( 'nhb-tab-tabs' ) ) {
			return false;
		}
	}
	return apply;
} );
wp.hooks.addFilter( 'nhb.overlay.margin_bottom.can_apply', function( apply, element ) {
	if ( element && element._domElement && element._domElement.parentNode && element._domElement.parentNode.classList ) {
		var parent = element._domElement.parentNode;
		if ( parent.classList && parent.classList.contains( 'nhb-tab-tabs' ) ) {
			return false;
	...
	ContentEdit.Text.prototype._keyUp = function( ev ) {
		...
		if ( this.parent().constructor.name === 'DivCol' && this.parent().children.indexOf(this) === 0 && selection.get()[0] === 0 ) {
			if ( this.parent().parent().parent().constructor.name === 'TabPanelContainer' ) {
				ev.preventDefault();
				var tabs = this.parent().parent().parent().parent()._domElement.querySelector( '.nhb-tab-tabs' )._ceElement;
...
(function() {
	...
	ContentEdit.Node.prototype.previous = function() {
		...
		if ( node && node._domElement && window.nhbSelectorMatches( node._domElement, '.nhb-tab-panels *' ) ) {
...
( function() {
    ...
    ContentEdit.DivCol.prototype._onMouseDown = function( ev ) {
		if ( this.parent().parent()._domElement.classList.contains( 'nhb-tab-panels' ) ) {
...
ContentEdit.Image.prototype.openMediaManager = function() {
	var frame = wp.media.editor.open( 'edit', {
		frame: 'image',
		state: 'image-details',
		metadata: _nhbImageGetMetaData( this._domElement )
	});

	frame.state('image-details').on( 'update', function( imageData ) {
		_nhbUpdateNonCaptionedImageCTElement( this, imageData );
	}.bind( this ) );
...
wp.hooks.addFilter( 'nhb.shortcode.allow_raw_edit', function( allow, scBase, element ) {
	if ( scBase === 'caption' ) {
		...
		var frame = wp.media.editor.open( 'edit', {
			frame: 'image',
			state: 'image-details',
			metadata: _nhbImageGetMetaData( target )
		});

		frame.state('image-details').on( 'update', function( imageData ) {
			_nhbUpdateNonCaptionedImage( target, imageData );
		} );
 ...
 ************************************************************************************/
var _nhbToolbarImageHasTextContent = function( node ) {
	return node && !! ( node.textContent || node.innerText );
};
var _nhbToolbarImageGetParent = function ( node, className ) {
...
var _nhbUpdateNonCaptionedImage = function( imageNode, imageData ) {
	...
	if ( imageNode.parentNode && imageNode.parentNode.nodeName === 'A' && ! _nhbToolbarImageHasTextContent( imageNode.parentNode ) ) {
		...
		var linkNode = _nhbToolbarImageGetParent( imageNode, 'a' );
		if ( linkNode ) {
	...
	captionNode = _nhbToolbarImageGetParent( imageNode, '.mceTemp' );
...
var _nhbUpdateNonCaptionedImageCTElement = function( imageNode, imageData ) {
...
var _nhbImageGetMetaData = function( img ) {
...
wp.hooks.addFilter( 'nhb.toolbar.shortcode.label', function( scBase ) {
	if ( scBase === 'nhb_widget' ) {
		return nhbParams.labels.widget;
	}
	return scBase;
} );
 ...
 */
wp.hooks.addFilter( 'nhb.inspector.do_add_section_options', function( doContinue, optionName, model, divGroup, element, toolboxUI ) {
	if ( optionName === 'widgetSettings' && model && model.attributes && model.attributes.widget ) {
		...
		doContinue = !! document.querySelector( '#tmpl-nhb-widget-' + model.attributes.widget );
		if ( ! doContinue ) {
...
/* globals nhbEditor, nhbParams */

/**
 * Usage:

	nhbEditor.openMediaManager( function( attachment ) {
...
window.addEventListener( 'DOMContentLoaded', function() {
	wp.media.frames.nhbSelectImage = wp.media({
		title: nhbParams.labels.select_image,
		multiple: false,
		library: {
			type: 'image'
		},
		button: {
			text: nhbParams.labels.use_selected_image
		}
	});

	nhbEditor.openMediaManager = function( callback, selectedImageID ) {
		wp.media.frames.nhbSelectImage.callback = function( ) {

			// Remove event listeners.
			wp.media.frames.nhbSelectImage.off('close', wp.media.frames.nhbSelectImage.callback);
			wp.media.frames.nhbSelectImage.off('select', wp.media.frames.nhbSelectImage.callback);

			// Get selected image.
		    var selection = wp.media.frames.nhbSelectImage.state().get( 'selection' );
		...
		wp.media.frames.nhbSelectImage.on('close', wp.media.frames.nhbSelectImage.callback);
		wp.media.frames.nhbSelectImage.on('select', wp.media.frames.nhbSelectImage.callback);

		wp.media.frames.nhbSelectImage._onOpen = function() {
			wp.media.frames.nhbSelectImage.off('open', wp.media.frames.nhbSelectImage._onOpen);
		...
		wp.media.frames.nhbSelectImage.on('open', wp.media.frames.nhbSelectImage._onOpen);

		wp.media.frames.nhbSelectImage.open();
	};
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	var appendWidgetTemplates = function( ajaxResponse ) {
		...
		localStorage.setItem( 'nhb_get_widget_templates_hash', nhbParams.widget_list_hash );
		localStorage.setItem( 'nhb_get_widget_templates', ajaxResponse );
	 ...
	 */
	var storedWidgetHash = localStorage.getItem( 'nhb_get_widget_templates_hash' );
	var storedWidgets = localStorage.getItem( 'nhb_get_widget_templates' );

	if ( storedWidgetHash === nhbParams.widget_list_hash && storedWidgets ) {
	 ...
	 */
	var payload = new FormData();
	payload.append( 'action', 'nhb_get_widget_templates' );
	payload.append( 'nonce', nhbParams.nonce );
...
/* globals ContentTools, ContentEdit, __extends, nhbParams */
...
ContentTools.ToolboxBarUI = ( function( _super ) {
	...
	ToolboxBarUI.prototype.mount = function() {
		...
		this._domElement = this.constructor.createDiv( [ 'nhb-toolbox-bar', 'ct-widget', 'ct-toolbox' ] );
		document.body.appendChild( this._domElement );
		_ref = this._tools;
		for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
			...
			for (_j = 0, _len1 = toolGroup.length; _j < _len1; _j++) {
				...
				if ( toolName === '|' ) {
					tool = this.constructor.createDiv(['nhb-tool-sep']);
...
( function() {
	...
	_EditorApp.prototype.init = function( queryOrDOMElements, namingProp ) {
		...
		this._toolboxBar = new ContentTools.ToolboxBarUI( window.nhbEditor.formattingTools );
 ...
 */
ContentTools.Tools.Bold.label = nhbParams.labels.bold;
ContentTools.Tools.Italic.label = nhbParams.labels.italic;
ContentTools.Tools.Link.label = nhbParams.labels.link;
ContentTools.Tools.AlignLeft.label = nhbParams.labels.align_left;
ContentTools.Tools.AlignCenter.label = nhbParams.labels.align_center;
ContentTools.Tools.AlignRight.label = nhbParams.labels.align_right;
ContentTools.Tools.OrderedList.label = nhbParams.labels.numbered_list;
ContentTools.Tools.UnorderedList.label = nhbParams.labels.bullet_list;
ContentTools.Tools.Indent.label = nhbParams.labels.indent;
ContentTools.Tools.Unindent.label = nhbParams.labels.unindent;
ContentTools.Tools.Undo.label = nhbParams.labels.undo;
ContentTools.Tools.Redo.label = nhbParams.labels.redo;
...
ContentTools.ToolboxFixedUI = ( function( _super ) {
	...
	ToolboxFixedUI.prototype.toggle = function() {
		if ( ! this._domElement ) {
			this.show();
		} else if ( this._domElement.classList.contains( 'nhb-toolbox-elements-shown' ) ) {
			this.hide();
		} else {
			this.show();
		}
	...
	ToolboxFixedUI.prototype.mount = function() {
		var domToolGroup, i, tool, toolGroup, toolName, _i, _j, _len, _len1, _ref;
		this._domElement = this.constructor.createDiv( [ 'nhb-interactive-elements-group', 'nhb-toolbox-elements', 'ct-widget', 'ct-toolbox' ] );
		document.body.appendChild( this._domElement );

		var note = this.constructor.createDiv( [ 'nhb-toolbox-elements-note' ] );
		note.innerHTML = nhbParams.labels.drag_an_element;
	...
	ToolboxFixedUI.prototype._addDOMEventListeners = function() {
		...
		this._domElement.addEventListener( 'mouseleave', function( ev ) {
			if ( ev.relatedTarget ) {
				if ( window.nhbSelectorMatches( ev.relatedTarget, '.ct-tool--insert-element, .ct-tool--insert-element *' ) ) {
					return;
				}
			}
			this.hide();
...
( function() {
	...
	_EditorApp.prototype.init = function( queryOrDOMElements, namingProp ) {
		...
		this._toolboxElements = new ContentTools.ToolboxFixedUI( window.nhbEditor.insertElements );
...
window.addEventListener( 'DOMContentLoaded', function() {
	document.querySelector( '.nhb-main-wrapper' ).addEventListener( 'mousedown', function() {
		ContentTools.EditorApp.get()._toolboxElements.hide();
	} );
	document.querySelector( '.nhb-main-wrapper' ).addEventListener( 'mouseup', function() {
		ContentTools.EditorApp.get()._toolboxElements.hide();
	} );
	document.querySelector( '.nhb-main-wrapper' ).addEventListener( 'keydown', function() {
		ContentTools.EditorApp.get()._toolboxElements.hide();
	} );
} );

/* globals ContentTools, ContentEdit, __extends, nhbEditor */


( function() {
	var proxied = ContentTools.ToolUI.prototype._onMouseLeave;
	ContentTools.ToolUI.prototype._onMouseLeave = function( ev ) {
		if ( this._mouseDown && this._domElement.classList.contains( 'nhb-tool-large' ) ) {
...
ContentEdit.NewElementDragHelper = ( function( _super ) {
    ...
    NewElementDragHelper.droppers = nhbEditor.allDroppers;
...
/* globals ContentTools, __extends, __bind, nhbInspectorOptions, nhbParams */
...
ContentTools.ToolboxPropertiesUI = (function(_super) {
    ...
    ToolboxPropertiesUI.prototype.mount = function() {
      ...
      this._domElement = this.constructor.createDiv(['nhb-toolbox-properties', 'ct-widget', 'ct-toolbox']);
      this.parent().domElement().appendChild(this._domElement);
      this._domGrip = this.constructor.createDiv( [ 'nhb-toolbox-titlebar' ] );
      this._domElement.appendChild( this._domGrip );
      this._domTabs = this.constructor.createDiv( [ 'nhb-toolbox-tabs' ] );
      this._domElement.appendChild( this._domTabs );
      this._domSections = this.constructor.createDiv( [ 'nhb-toolbox-sections' ] );
      this._domElement.appendChild( this._domSections );
      this._domResize = this.constructor.createDiv( [ 'nhb-toolbox-resizer' ] );
      this._domElement.appendChild( this._domResize );
	  var title = this.constructor.createDiv( [ 'nhb-toolbox-title' ] );
	  title.innerHTML = nhbParams.labels.properties_inspector;
      this._domGrip.appendChild( title );
	  this._closeButton = this.constructor.createDiv( [ 'nhb-titlebox-close' ] );
	  ...
	  if ( window.localStorage.getItem( 'nhb-toolbox-elements-position' ) === null ) {
		  window.localStorage.setItem( 'nhb-toolbox-elements-position', ( window.innerWidth - 250 - 30 ) + ',87,' + ( window.innerHeight - 77 - 20 ) );
	  }
      restore = window.localStorage.getItem('nhb-toolbox-elements-position');
	...
	ToolboxPropertiesUI.prototype._onResizeStop = function( ev ) {
		...
		window.localStorage.setItem( 'nhb-toolbox-elements-position', parseInt( rect.left, 10 ) + ',' + parseInt( rect.top, 10 ) + ',' + parseInt( rect.height, 10 ) );
    ...
    ToolboxPropertiesUI.prototype._contain = function() {
		...
		if ( document.querySelector('.nhb-toolbox-bar') ) {
			top += parseInt( document.querySelector('.nhb-toolbox-bar').clientHeight, 10 );
		}
	...
	ToolboxPropertiesUI.prototype._addTab = function( label, element ) {
		...
		tab.classList.add( 'nhb-toolbox-tab' );
		tab.innerHTML = label;
		...
		if ( this._domTabs.children.length === 1 ) {
			tab.classList.add( 'nhb-toolbox-tab-shown' );
		}

		// Add event handlers.
		tab.addEventListener( 'click', function() {
			var visibleTab = this.parentNode.querySelector( '.nhb-toolbox-tab-shown' );
			if ( visibleTab ) {
				visibleTab.classList.remove( 'nhb-toolbox-tab-shown' );
			}
			this.classList.add( 'nhb-toolbox-tab-shown' );

			var visibleSection = this.parentNode.parentNode.querySelector( '.nhb-toolbox-section-shown' );
			if ( visibleSection ) {
				visibleSection.classList.remove( 'nhb-toolbox-section-shown' );
			}
			this.parentNode.parentNode.querySelector( '.nhb-toolbox-section[data-name="' + this.getAttribute( 'data-name' ) + '"]' ).classList.add( 'nhb-toolbox-section-shown' );
	...
	ToolboxPropertiesUI.prototype.addSection = function( label, element, optionIndex ) {
		...
		section.classList.add( 'nhb-toolbox-section' );
		...
		if ( this._domSections.children.length === 1 ) {
			section.classList.add( 'nhb-toolbox-section-shown' );
		}
		...
		if ( element.constructor.name === 'Shortcode' ) {
			...
			if ( typeof nhbInspectorOptions.Shortcode[ element.sc_base ] === 'undefined' ) {
				if ( typeof nhbParams.shortcode_mappings !== 'undefined' && typeof nhbParams.shortcode_mappings[ element.sc_base ] !== 'undefined' ) {
		...
		if ( elemType === 'Shortcode' ) {
			...
			var shortcodeProperties = nhbInspectorOptions.Shortcode[ shortcodeBase ];
			...
			if ( typeof shortcodeProperties !== 'undefined' && typeof shortcodeProperties.options !== 'undefined' ) {
				...
				if ( numWithGroups && numNoGroup ) {
					for ( k = 0; k < shortcodeProperties.options.length; k++ ) {
						if ( ! shortcodeProperties.options[ k ].group ) {
							shortcodeProperties.options[ k ].group = nhbParams.labels.general;
						}
					}
				}
				...
				if ( ! shortcodeProperties.options.length ) {
					...
					note.innerHTML = nhbParams.labels.no_attributes_available;
					note.classList.add( 'nhb-shortcode-no-options' );
				...
				heading.innerHTML = nhbParams.labels.note_options_are_detected;
				section.appendChild( heading );
				// heading.innerHTML += '<span>' + nhbParams.labels.note_options_are_detected + '</span>';
			...
			shortcodeNote.classList.add( 'nhb-inspector-shortcode-note' );
			shortcodeNote.innerHTML = nhbParams.labels.note_shortcode_not_appearing;
			section.appendChild( shortcodeNote );

		} else if ( typeof nhbInspectorOptions[ optionIndex ] !== 'undefined' ) {
			...
			numWithGroups = 0;
			numNoGroup = 0;
			for ( k = 0; k < nhbInspectorOptions[ optionIndex ].options.length; k++ ) {
				if ( nhbInspectorOptions[ optionIndex ].options[ k ].group ) {
					numWithGroups++;
				} else {
					numNoGroup++;
				}
			...
			if ( numWithGroups && numNoGroup ) {
				for ( k = 0; k < nhbInspectorOptions[ optionIndex ].options.length; k++ ) {
					if ( ! nhbInspectorOptions[ optionIndex ].options[ k ].group ) {
						nhbInspectorOptions[ optionIndex ].options[ k ].group = nhbParams.labels.general;
					}
				}
			}

			for ( k = 0; k < nhbInspectorOptions[ optionIndex ].options.length; k++ ) {
				this.addSectionOptions( section, nhbInspectorOptions[ optionIndex ].options[ k ], element, currElemModel );
			}

			// Add hover events if there are any.
			if ( nhbInspectorOptions[ optionIndex ].onMouseEnter ) {
				section.addEventListener( 'mouseenter', function( e ) {
					nhbInspectorOptions[ optionIndex ].onMouseEnter( this, e );
				}.bind( element ) );
			}
			if ( nhbInspectorOptions[ optionIndex ].onMouseLeave ) {
				section.addEventListener( 'mouseleave', function( e ) {
					nhbInspectorOptions[ optionIndex ].onMouseLeave( this, e );
				}.bind( element ) );
			}

			// Footer notice for the inspector.
			if ( typeof nhbInspectorOptions[ optionIndex ].footer !== 'undefined' ) {
				if ( Math.random() > 0.5 ) { // Do this only half of the time.
					note = document.createElement( 'DIV' );
					note.innerHTML = nhbInspectorOptions[ optionIndex ].footer;
					note.classList.add( 'nhb-group-footer' );
					section.appendChild( note );
				}
			}
		}
	...
	ToolboxPropertiesUI.prototype.addOptions = function( element ) {
		...
		if ( ! window.nhbInspectorOptions[ element.constructor.name ] && element.nodeType ) {
			// Check if the element matches any of the selectors.
			var matched = false;

			for ( var index in window.nhbInspectorOptions ) {
				if ( window.nhbInspectorOptions.hasOwnProperty( index ) ) {
					if ( window.nhbSelectorMatches( element, index ) ) {
						optionIndex = index;
						matched = true;
						break;
					}
		...
		if ( ! window.nhbInspectorOptions[ optionIndex ] ) {
			return;
		}

		label = window.nhbInspectorOptions[ optionIndex ].label;

		// Shortcodes are just labeled 'Shortcode'.
		if ( element.constructor.name === 'Shortcode' ) {
			label = element.constructor.name;

			if ( element.sc_base === 'nhb_widget' ) {
				label = 'Widget';
			} else if ( element.sc_base === 'nhb_sidebar' ) {
				label = 'Sidebar';
			}
		}
...
/* globals ContentEdit, ContentTools, nhbOption, nhbParams */
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	// editor.bind('stop', function() {
	// 	document.querySelector('html').classList.remove('nhb-inspector-docked-right');
	// 	document.querySelector('html').classList.remove('nhb-inspector-docked-left');
	// });
	...
	toolbox._contain = function() {
		...
		if ( document.querySelector('.nhb-toolbox-bar') ) {
			formatBar = parseInt( document.querySelector('.nhb-toolbox-bar').clientHeight, 10 );
		}
		...
		if ( window.innerWidth - this._domElement.offsetWidth <= parseInt( this._domElement.style.left, 10 ) + 5 ) {
			...
			document.querySelector('html').classList.add('nhb-inspector-docked-right');
			document.querySelector('html').classList.remove('nhb-inspector-docked-left');
		} else if ( 5 >= parseInt( this._domElement.style.left, 10 ) ) {
			this._domElement.style.left = '0px';
			document.querySelector('html').classList.add('nhb-inspector-docked-left');
			document.querySelector('html').classList.remove('nhb-inspector-docked-right');
		} else {
			document.querySelector('html').classList.remove('nhb-inspector-docked-right');
			document.querySelector('html').classList.remove('nhb-inspector-docked-left');
	...
	*/
	...
	editor.bind('start', function() {
		...
		jQuery( 'body' ).on( 'DOMMouseScroll mousewheel', '.nhb-toolbox-elements, .nhb-toolbox-sections', stopBodyScroll );
	});
	editor.bind('stop', function() {
		jQuery( 'body' ).off( 'DOMMouseScroll mousewheel', '.nhb-toolbox-elements, .nhb-toolbox-sections', stopBodyScroll );
	});
});
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	document.addEventListener( 'mouseup', function(ev) {
		...
		if ( window.nhbSelectorMatches( ev.target, '[data-name="main-content"] *' ) ) {
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	document.addEventListener( 'mousedown', function(ev) {
		...
		// Only entertain clicks on the editor area.
		if ( window.nhbSelectorMatches( ev.target, '[data-name="main-content"] *, .nhb-quick-action-overlay, .nhb-quick-action-overlay *' ) ) {
...
/*
ContentTools.ToolboxUI.prototype._nhbAddDesignMount = ContentTools.ToolboxUI.prototype.mount;
ContentTools.ToolboxUI.prototype.mount = function() {
	var ret = this._nhbAddDesignMount();

	for ( var i = 0; i < window.nhbEditor.toolHeadings.length; i++ ) {
		var toolHeader = window.nhbEditor.toolHeadings[ i ];
		var label = toolHeader.label;
		var cls = 'nhb-' + toolHeader['class'];
		...
		heading.classList.add( 'nhb-group-title' );
		heading.classList.add( 'nhb-collapsable-title' );
		...
		if ( toolHeader.tip ) {
			heading.appendChild( window.nhbEditor.createGroupTip( toolHeader.tip ) );
		}
	}
...
window.nhbEditor.createGroupTip = function( text ) {
	var tip = document.createElement( 'span' );
	tip.classList.add( 'nhb-group-tip' );
	tip.innerHTML = '?';
	var tipText = document.createElement( 'span' );
	tipText.classList.add( 'nhb-group-tip-details' );
...
ContentTools.ToolboxPropertiesUI.prototype.addSectionOptions = function( divGroup, option, element, model ) {
	...
	if ( typeof option.type !== 'undefined' ) {
		matchesAnOptionType = Object.keys( nhbOption ).some( function( name ) {
	...
	for ( optionName in nhbOption ) {
		if ( nhbOption.hasOwnProperty( optionName ) && type === optionName.toLowerCase() ) {
			...
			if ( type !== 'button' ) {
				optionWrapper.classList.add( 'nhb-tool-option' );
			}
			if ( option['class'] ) {
				optionWrapper.setAttribute( 'class', optionWrapper.getAttribute('class') + ' ' + option['class'] );
			}
			if ( optionName ) {
				optionWrapper.classList.add( 'nhb-' + optionName.replace( /([a-z])([A-Z])/g, '$1-$2' ).toLowerCase() );
			}
			if ( option.id ) {
			...
			if ( option.group ) {
				...
				divGroup.classList.add( 'nhb-has-group' );
				if ( ! subGroup ) {
					...
					if ( option.group !== 'default' ) {
						subGroup.innerHTML = '<div class="nhb-option-subtitle nhb-tool-option">' + option.group + '</div>';

						// For shortcodes, groupings are accordions. Make it collapsable.
						// if ( element.constructor.name === 'Shortcode' ) {
							subGroup.firstChild.classList.add( 'nhb-collapsable-title' );
							...
							if ( divGroup.querySelectorAll( '[data-subgroup]' ).length > 1 ) {
								window.nhbCollapseSection( subGroup );
							}
						// }
					}
				}
				...
				if ( option['group-tip'] ) {
					if ( subGroup.querySelector( '.nhb-group-tip-details' ) ) {
						subGroup.querySelector( '.nhb-group-tip-details' ).innerHTML = option['group-tip'];
					} else {
						subGroup.querySelector( '.nhb-option-subtitle' ).appendChild( window.nhbEditor.createGroupTip( option['group-tip'] ) );
					}
				}
			...
			if ( ! wp.hooks.applyFilters( 'nhb.inspector.do_add_section_options', true, optionName, model, divGroup, element, this ) ) {
				return;
			}

			// this._domInspector.insertBefore( optionWrapper, this._domInspector.firstChild );
			var o = new nhbOption[ optionName ]({
				optionSettings: option,
				model: model,
				el: optionWrapper
			});
			_nhbCreatedViews.push( o.render() );
...
ContentTools.ToolboxPropertiesUI.prototype.addGenericShortcodeOptions = function( divGroup, element ) {
	...
	divGroup.classList.add( 'nhb-shortcode-generic' );
	...
	for ( i = 0; i < keys.length; i++ ) {
		...
		if ( attributeName !== 'content' ) {
			...
			view = new nhbOption.GenericOption({
				attribute: attributeName,
				model: element.model
			});
			_nhbCreatedViews.push( view.render() );
	...
	if ( ! hasOptions ) {
		...
		note.innerHTML = nhbParams.labels.shortcodes_not_attributes_detected;
		note.classList.add( 'nhb-shortcode-no-options' );
...
ContentTools.ToolboxPropertiesUI.prototype._addSection = function( domElement ) {
	...
	if ( domElement ) {
		...
		while ( domElement ) {
			if ( domElement.tagName && domElement.tagName !== 'DIV' ) {
				if ( window.nhbSelectorMatches( domElement, '[data-name="main-content"] *' ) ) {
		...
		label = '';
		if ( currDomElement ) {
			for ( var pattern in nhbInspectorOptions ) {
				if ( nhbInspectorOptions.hasOwnProperty( pattern ) ) {
					try {
						if ( window.nhbSelectorMatches( domElement, pattern ) ) {
		...
		if ( matchedPattern ) {
			if ( nhbInspectorOptions[ matchedPattern ].label ) {
				label = nhbInspectorOptions[ matchedPattern ].label;
			}
			if ( ! label ) {
			...
			// Create the group.
			group = this.constructor.createDiv( ['ct-tool-group', 'nhb-inspector-group', 'nhb-dom-group'] );
			...
			heading.classList.add( 'nhb-group-title' );
			heading.classList.add( 'nhb-collapsable-title' );
			heading.classList.add( 'nhb-dom-title' );
			heading.innerHTML = nhbParams.labels.inspector_title.replace( '%s', label );
			group.insertBefore( heading, group.firstChild );
			...
			for ( i = 0; i < nhbInspectorOptions[ matchedPattern ].options.length; i++ ) {
				this.addSectionOptions( group, nhbInspectorOptions[ matchedPattern ].options[i], currDomElement, currElemModel );
			}
		}
	}
	...
	for ( i = 0; i < hierarchy.length; i++ ) {
		...
		elemType = wp.hooks.applyFilters( 'nhb.inspector.elemtype', elemType, currElem );
		...
		if ( elemType === 'Shortcode' ) {
			...
			// If there is an existing shortcode mapping, use that.
			if ( typeof nhbInspectorOptions.Shortcode[ currElem.sc_base ] === 'undefined' ) {
				if ( typeof nhbParams.shortcode_mappings !== 'undefined' && typeof nhbParams.shortcode_mappings[ currElem.sc_base ] !== 'undefined' ) {
					if ( this.createShortcodeMappingOptions ) {
			...
			if ( typeof nhbInspectorOptions.Shortcode[ currElem.sc_base ] !== 'undefined' && typeof nhbInspectorOptions.Shortcode[ currElem.sc_base ].label !== 'undefined' ) {
				label = nhbInspectorOptions.Shortcode[ currElem.sc_base ].label;
			}

		} else if ( typeof nhbInspectorOptions[ elemType ] !== 'undefined' && doneTypes.indexOf( elemType ) === -1 ) {

			label = currElem.typeName();
			if ( nhbInspectorOptions[ elemType ].label ) {
				label = nhbInspectorOptions[ elemType ].label;
			}
		...
		// Create the group.
		groupClasses = ['ct-tool-group', 'nhb-inspector-group', 'nhb-' + currElem.cssTypeName() + '-group'];
		if ( elemType === 'Shortcode' ) {
			groupClasses.push( 'nhb-shortcode-' + currElem.sc_base + '-group' );
		}
		...
		heading.classList.add( 'nhb-group-title' );
		heading.classList.add( 'nhb-collapsable-title' );
		heading.classList.add( 'nhb-' + currElem.cssTypeName() + '-title' );
		heading.innerHTML = nhbParams.labels.inspector_title.replace( '%s', label );
		group.insertBefore( heading, group.firstChild );
		...
		if ( elemType === 'Shortcode' ) {
			...
			shortcodeProperties = nhbInspectorOptions.Shortcode[ shortcodeBase ];
			...
			for ( k = 0; k < nhbInspectorOptions[ elemType ].options.length; k++ ) {
				this.addSectionOptions( group, nhbInspectorOptions[ elemType ].options[ k ], currElem, currElemModel );
			}

			// Footer notice for the inspector.
			if ( typeof nhbInspectorOptions[ elemType ].footer !== 'undefined' ) {
				if ( Math.random() > 0.5 ) { // Do this only half of the time.
					note = document.createElement( 'DIV' );
					note.innerHTML = nhbInspectorOptions[ elemType ].footer;
					note.classList.add( 'nhb-group-footer' );
					group.appendChild( note );
				}
			}

			wp.hooks.doAction( 'nhb.inspector.add_section', group, label );
...
ContentTools.ToolboxPropertiesUI.prototype.clearSections = function() {
	...
	while ( _nhbCreatedViews.length > 0 ) {
		var o = _nhbCreatedViews.pop();
		o.remove();
	}
	...
	for ( var i = 0; i < this._oldGroups.length; i++ ) {
		...
		if ( oldGroup.groupType === group.groupType ) {
			...
			setTimeout( function() { // jshint ignore:line
				...
				if ( this.classList.contains('nhb-collapse') ) {
					endHeight = getComputedStyle( this.querySelector( '.nhb-group-title' ) ).height;
				}
 ...
 ***********************************************************************************************/
window.nhbElementsWithInspector = [];
window.nhbAddInspector = function( elemName, args ) {// options ) {
	if ( typeof args !== 'object' ) {
		return;
	}
	var i, container = nhbInspectorOptions;

	// Support multiple element names given.
	if ( typeof elemName === 'object' ) {
		for ( i = 0; i < elemName.length; i++ ) {
			window.nhbAddInspector( elemName[ i ], args );
		}
		return;
	}
	...
	if ( typeof args.is_shortcode !== 'undefined' ) {
		if ( args.is_shortcode ) {
			container = nhbInspectorOptions.Shortcode;
		} else {
			if ( window.nhbElementsWithInspector.indexOf( elemName ) === -1 ) {
				window.nhbElementsWithInspector.push( elemName );
			}
		}
	} else {
		if ( window.nhbElementsWithInspector.indexOf( elemName ) === -1 ) {
...
window.nhbRemoveInspector = function( elemName ) {
	var i, container = nhbInspectorOptions;

	// Support multiple element names given.
	if ( typeof elemName === 'object' ) {
		for ( i = 0; i < elemName.length; i++ ) {
			window.nhbRemoveInspector( elemName[ i ] );
		}
		return;
	}

	if ( typeof nhbInspectorOptions.Shortcode[ elemName ] !== 'undefined' ) {
		container = nhbInspectorOptions.Shortcode;
	}
 ...
 *********************************************************************/
if ( nhbParams.additional_shortcodes ) {
	for ( var elemName in nhbParams.additional_shortcodes ) {
		if ( nhbParams.additional_shortcodes.hasOwnProperty( elemName ) ) {
			nhbInspectorOptions.Shortcode[ elemName ] = nhbParams.additional_shortcodes[ elemName ];
		}
	}
}
 ...
 */
window.nhbCollapseSection = function( section ) {
	var classes;

	// Do collapse animation.
	if ( section.classList.contains( 'nhb-collapse' ) ) {
		...
		if ( section.getAttribute( 'data-collapse-group' ) ) {
			var openSections = document.querySelectorAll( '[data-collapse-group="' + section.getAttribute( 'data-collapse-group' ) + '"]:not(.nhb-collapse)' );
			Array.prototype.forEach.call( openSections, function(el) {
				window.nhbCollapseSection( el );
			});
		}
		...
		section.classList.remove( 'nhb-collapse' );

		classes = section.getAttribute( 'class' );
		classes = classes.replace( /\s*(ct-tool-group|nhb-collapse|nhb-inspector-group)\s*/g, '' );
		...
		section.classList.add( 'nhb-collapse' );

		classes = section.getAttribute( 'class' );
		classes = classes.replace( /\s*(ct-tool-group|nhb-collapse|nhb-inspector-group)\s*/g, '' );
	}
};
window.nhbOpenAllSections = function() {
	var sections = document.querySelectorAll( '.ct-tool-group' );
	Array.prototype.forEach.call( sections, function(el){
		if ( el.classList.contains('nhb-collapse') ) {
			window.nhbCollapseSection( el );
		}
	});
};
window.nhbOnlyOpenSection = function( sectionClass ) {
	var sections = document.querySelectorAll( '.ct-tool-group' );
	Array.prototype.forEach.call( sections, function(el){
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	// var editor = ContentTools.EditorApp.get();
	// editor.bind( 'start', function() {
	// 	if ( typeof localStorage.getItem( 'nhb_collapsed_sections' ) !== 'undefined' && localStorage.getItem( 'nhb_collapsed_sections' ) ) {
	// 		storedCollapsed = JSON.parse( localStorage.getItem( 'nhb_collapsed_sections' ) );
	//
	...
	document.addEventListener('click', function(ev) {
		...
		if ( ! ev.target.classList.contains( 'nhb-collapsable-title' ) ) {
			...
			if ( ! ev.target.parentNode.classList.contains( 'nhb-collapsable-title' ) ) {
				return;
			}
		...
		window.nhbCollapseSection( section );
	});
});
...
/* globals ContentEdit, ContentTools, nhbParams */


var nhbOption = {};


var nhbBaseView = Backbone.View.extend( {
	...
	initialize: function(options) {
		...
		if ( this.optionSettings.visible ) {
			...
			wp.hooks.addAction( 'nhb.option.changed', this._visibleBound );
			this._visible();
		}
	...
	remove: function() {
		if ( this.optionSettings.visible ) {
			wp.hooks.removeAction( 'nhb.option.changed', this._visibleBound );
		}
...
nhbOption.widgetSettings = Backbone.View.extend({

	className: 'nhb-tool-option',
	...
	initialize: function(options) {
		...
		this.template = wp.template( 'nhb-widget-' + this.model.attributes.widget );
	},
	...
	render: function() {
		...
		var widgetInfo = nhbParams.widget_list[ this.model.attributes.widget ];
		// var inspectorContainer = this.el.parentNode.querySelector( '.nhb-group-title' );
		// inspectorContainer.innerHTML = nhbParams.labels.widget_properties_title.replace( '%s', widgetInfo.name ) + '<span>' + widgetInfo.description + '</span>';
...
nhbOption.Button = nhbBaseView.extend({
	...
	initialize: function(options) {
		nhbBaseView.prototype.initialize.call( this, options );
	...
	updateTooltip: function( value ) {
		...
		} else if ( window.nhbEditor.isCtrlDown ) {
...
nhbOption.GenericOption = Backbone.View.extend({
	template: wp.template( 'nhb-shortcode-generic-option' ),
	...
	initialize: function(options) {
		...
		if ( this.attribute === 'content' ) {
			this.template = wp.template( 'nhb-shortcode-generic-content' );
		}
	},
...
nhbOption.Border = Backbone.View.extend({
	template: wp.template( 'nhb-option-border' ),
	...
	events: {
		...
		'click .nhb-color-preview': 'togglePicker',
		'change .nhb-color-popup input' : 'colorChanged',
		'keyup .nhb-color-popup input' : 'colorChanged'
	},
	...
	colorChanged: function() {
		var inputColor = this.el.querySelector('.nhb-color-popup input').value;
		...
		this.el.querySelector('.nhb-color-preview').style.background = color;
	},
	...
	togglePicker: function() {
		if ( this.el.querySelector('.nhb-color-popup').style.display === 'block' ) {
			this.el.querySelector('.nhb-color-popup').style.display = '';
		} else {
			this.el.querySelector('.nhb-color-popup').style.display = 'block';
		}
	},
...
nhbOption.Color = nhbBaseView.extend({
	template: wp.template( 'nhb-option-color' ),
	...
	updateColor: function( color ) {
		if ( this.el.querySelector('.nhb-color-preview').style.background !== color ) {
			this.el.querySelector('.nhb-color-preview').style.background = color;
	...
	*/
	...
	render: function() {
	    ...
	    this.$el.html( this.template( data ) );
		// this.el.classList.add( 'nhb-button' );
	...
	remove: function() {
        ...
        nhbBaseView.prototype.remove.call( this );
    },
	...
	selectChanged: function( forceColor ) {
		...
		wp.hooks.doAction( 'nhb.option.changed' );
	},
	...
	togglePicker: function() {
		...
		var popup = this.el.querySelector('.nhb-color-popup');
		var otherPopups = document.querySelectorAll( '.nhb-color-popup' );
		...
		if ( popup.style.display === 'block' ) {
			...
			wp.hooks.doAction( 'nhb.tool.popup.open' );
			popup.style.display = 'block';
		}

		// Close popup if other popups open.
		wp.hooks.addAction( 'nhb.tool.popup.open', function() {
			popup.style.display = '';
		}.bind(this));
	},
...
nhbOption.ColorButton = nhbOption.Color.extend({});


nhbOption.Select = nhbBaseView.extend({
	template: wp.template( 'nhb-option-select' ),
...
nhbOption.Checkbox = nhbBaseView.extend({
	template: wp.template( 'nhb-option-checkbox' ),
...
nhbOption.Text = nhbBaseView.extend({
	template: wp.template( 'nhb-option-text' ),
...
nhbOption.Textarea = nhbOption.Text.extend({
	template: wp.template( 'nhb-option-textarea' ),
...
nhbOption.Number = nhbOption.Text.extend( {
	template: wp.template( 'nhb-option-number' ),
	...
	render: function() {
		...
		if ( this.$el.html() ) {
			...
			if ( this.optionSettings.value ) {
				...
				var slider = this.$el.find( '.nhb-option-number-slider' );
		...
		nhbOption.Number.__super__.render.call( this );

		input = this.$el.find( 'input' );
		this.$el.find( '.nhb-option-number-slider' ).slider({
	...
	selectChanged: function( e ) {
		nhbOption.Number.__super__.selectChanged.call( this, e );
...
nhbOption.Image = nhbOption.Text.extend( {
	template: wp.template( 'nhb-option-image' ),

	multiple: false,

	events: {
		'change input' : 'selectChanged',
		'click .nhb-image-preview' : 'openImagePicker',
		'click .nhb-image-preview-remove': 'removeImage'
	},
	...
	render: function() {
		...
		var ret = nhbOption.Image.__super__.render.call( this );
		...
		if ( ajaxToGetImageURL ) {
			...
			this.$el.find( '.nhb-image-preview:not([data-id=""])' ).each( function() {
				var imageID = this.getAttribute( 'data-id' );
				if ( ! imageID.match( /^[\d,]+$/ ) ) {
					this.style.backgroundImage = imageID.match( /url\(/ ) ? imageID : 'url(' + imageID + ')';
					return;
				}
				if ( nhbOption.Image._imageURLs.hasOwnProperty( imageID ) ) {
					this.style.backgroundImage = 'url(' + nhbOption.Image._imageURLs[ imageID ] + ')';
				} else {
			...
			if ( imageIDsToAjaxLoad ) {
				nhbOption.Image.doAjaxToGetImageURLs( imageIDsToAjaxLoad );
			}
	...
	openImagePicker: function() {
		var frame = wp.media( {
			title: nhbParams.labels.s_attribute.replace( '%s', this.optionSettings.name ),
			multiple: this.multiple,
			library: { type: 'image' },
			button : { text : nhbParams.labels.select_image }
		});
		...
		frame.on('select', function() {
			...
			this.$el.find( '.nhb-image-preview' ).remove();
			...
			selection.each( function( attachment ) {
				...
				jQuery( '<div></div>' )
					.addClass( 'nhb-image-preview' )
					.css( 'backgroundImage', 'url(' + image.url + ')' )
					.attr( 'data-id', attachment.id )
					.append( jQuery( '<div></div>' ).addClass( 'nhb-image-preview-remove' ) )
					.appendTo( this.$el );
				...
				nhbOption.Image._imageURLs[ attachment.id ] = image.url;
	...
	removeImage: function(e) {
		...
		if ( this.multiple ) {
			this.$el.find( '.nhb-image-preview' ).each( function() {
		...
		if ( ! this.multiple || ( this.multiple && ! this.$el.find( '.nhb-image-preview' ).length ) ) {
			jQuery( '<div></div>' )
				.addClass( 'nhb-image-preview' )
				.attr( 'data-id', '' )
				.appendTo( this.$el );
...
nhbOption.Image._imageURLs = {};


// This is called to get the image URLs from a given set of
// comma separated attachment IDs.
nhbOption.Image.doAjaxToGetImageURLs = function( imageIDs ) {
	if ( typeof nhbOption.Image._imageIDsToAjax === 'undefined' ) {
	...
	nhbOption.Image._imageIDsToAjax += nhbOption.Image._imageIDsToAjax ? ',' : '';
	nhbOption.Image._imageIDsToAjax += imageIDs;

	// Do this in a timeout to only do one ajax at a time for faster querying.
	clearTimeout( nhbOption.Image._doAjaxToGetImageURLsTimeout );
	nhbOption.Image._doAjaxToGetImageURLsTimeout = setTimeout( function() {
		nhbOption.Image._doAjaxToGetImageURLs();
	}, 50 );
};
nhbOption.Image._doAjaxToGetImageURLs = function() {

	var imageIDs = nhbOption.Image._imageIDsToAjax || '';
	imageIDs = imageIDs.split( ',' );
	for ( var i = 0; i < imageIDs.length; i++ ) {
		jQuery( '.nhb-tool-option .nhb-image-preview[data-id=' + imageIDs[ i ] + ']' ).addClass( 'nhb-loading' );
	}

	var payload = new FormData();
	payload.append( 'action', 'nhb_get_attachment_urls' );
	payload.append( 'image_ids', nhbOption.Image._imageIDsToAjax );
	payload.append( 'nonce', nhbParams.nonce );
	...
	request.onload = function() {
			...
			try {
				...
				for ( i = 0; i < imageIDs.length; i++ ) {
					if ( typeof response[ imageIDs[ i ] ] !== 'undefined' ) {
						...
						jQuery( '.nhb-tool-option .nhb-image-preview[data-id=' + imageIDs[ i ] + ']' ).css( 'backgroundImage', 'url(' + response[ imageIDs[ i ] ] + ')' );

						// Keep the image for future use.
						nhbOption.Image._imageURLs[ imageIDs[ i ] ] = response[ imageIDs[ i ] ];

					} else {
						// We didn't have an image for this image ID.
						jQuery( '.nhb-tool-option .nhb-image-preview[data-id=' + imageIDs[ i ] + ']' ).addClass( 'nhb-ajax-error' );
					}
				}
		...
		for ( i = 0; i < imageIDs.length; i++ ) {
			jQuery( '.nhb-tool-option .nhb-image-preview[data-id=' + imageIDs[ i ] + ']' ).removeClass( 'nhb-loading' );
		}
...
nhbOption.Images = nhbOption.Image.extend( {
	multiple: 'toggle'
} );

nhbOption.MarginsAndPaddings = Backbone.View.extend({
	template: wp.template( 'nhb-option-margins-and-paddings' ),
...
nhbOption.CustomClass = Backbone.View.extend({
	template: wp.template( 'nhb-option-text' ),
...
nhbOption.CustomID = Backbone.View.extend({
	template: wp.template( 'nhb-option-text' ),
...
nhbOption.Button2 = nhbBaseView.extend({
	template: wp.template( 'nhb-option-button2' ),
...
options.push(

	{
		...
		'name': nhbParams.labels.icon_frame_change_title,

		'button': nhbParams.labels.pick_an_icon,

		'type': 'button2',

		'group': nhbParams.labels.general,

		'click': function( element ) {

			nhbEditor.iconFrame.open({

				title: nhbParams.labels.choose_icon,

				button: nhbParams.labels.choose_icon,
...

	{
		...
		'name': nhbParams.labels.icon_color,
...
window.nhbAddInspector( 'Icon', {

	'label': nhbParams.labels.icon,

	'footer': nhbParams.labels.icon_lite_footer, // LITE-ONLY
...
window.nhbAddInspector('nhb_widget', {
	'is_shortcode': true,
	'label': nhbParams.labels.widget,
	'desc': nhbParams.labels.widget_inspector_desc,
	'options': [
		{
			'type': 'widgetSettings'
		}
...
//
//
// window.nhbAddInspector('divider_advanced', {
...
// });
//
// window.nhbAddInspector('divider_arrow', {
...
window.nhbAddInspector('nhb_sidebar', {
	'is_shortcode': true,
	'label': nhbParams.labels.sidebar,
	'desc': nhbParams.labels.sidebar_inspector_desc,
	'options': [
		{
			'type': 'select',
			'name': nhbParams.labels.select_a_sidebar,
			'id': 'id',
			'options': nhbParams.sidebar_list,
			'desc': nhbParams.sidebar_label_id
		}
	]
});
...
/* globals nhbParams, Color, ContentEdit */
...
options.push(

	{
		...
		'name': nhbParams.labels.row_width,

		'desc': nhbParams.labels.desc_row_width,
		...
		'options': {
			...
			'': nhbParams.labels.normal_width,

			'full-width-retain-content': nhbParams.labels.full_width_retained_content_width,

			'full-width': nhbParams.labels.full_width
		...
		'getRootRow': function( element ) {
			...
			while ( currElem && currElem._ceElement ) {
				...
				if ( currElem.classList.contains( 'nhb-row' ) ) {
		...
		'change': function( element, value, view ) {
			...
			window._nhbFixRowWidth( rootRow._domElement );
...

	{
		...
		'name': nhbParams.labels.full_height,
...

	{
		...
		'name': nhbParams.labels.background_color,

		'type': 'color',

		'group': nhbParams.labels.background,
...

	{
		...
		'name': nhbParams.labels.background_image,
...
options.push(

	{
		...
		'name': nhbParams.labels.border_style,

		'type': 'select',

		'group': nhbParams.labels.borders,

		'options': {

			'': nhbParams.labels.none,

			'solid': nhbParams.labels.solid,

			'dashed': nhbParams.labels.dashed,

			'dotted': nhbParams.labels.dotted
...

	{
		...
		'name': nhbParams.labels.border_color,
...

	{
		...
		'name': nhbParams.labels.border_thickness,
...

	{
		...
		'name': nhbParams.labels.border_radius,
...
ContentTools.ToolboxUI.prototype.createShortcodeMappingOptions = function( shortcodeTag ) {
	...
	for ( var attribute in map.attributes ) {
		...
		if ( 'boolean' === options.type ) {
			...
			options.options = nhbParams.post_types;
		} else if ( 'dropdown' === options.type ) {
			options.type = 'select';
			options.options = {
				'': '— ' + nhbParams.labels.select_one + ' —'
			};
			opts = options.extra_dropdown.split( /\n/ ) || {};
			for ( i = 0; i < opts.length; i++ ) {
				value = opts[ i ].substr( 0, opts[ i ].indexOf( ',' ) );
			...
			options.type = 'select';
			options.options = {
				'': '— ' + nhbParams.labels.select_a_post_type + ' —'
			};
			for ( i in nhbParams.post_types ) {
				options.options[ i ] = nhbParams.post_types[ i ];
			}
		} else if ( 'dropdown_post' === options.type ) {
			...
			options.button = options.extra_button || nhbParams.labels.open;
	...
	window.nhbAddInspector( shortcodeTag, {
		'is_shortcode': true,
		'label': name,
...
/* globals google, nhbParams, nhbEditor */

var options = [];

options.push(
	{
		'name': nhbParams.labels.latitude_longitude_and_address,
		'type': 'text',
		'desc': nhbParams.labels.latitude_longitude_desc,
		'initialize': function( element, view ) {
		...
		'change': _.debounce( function( element, value, view ) {
			...
			view.$el.find( 'input' ).removeClass( 'nhb-option-error' );
			...
			if ( latLonMatch ) {
				...
				geocoder.geocode( { 'address': center }, function( results, status ) {
					if ( status === google.maps.GeocoderStatus.OK ) {
						...
						view.$el.find( 'input' ).addClass( 'nhb-option-error' );
					}
				});
			}
		}, 300 )
	...
	{
		'name': nhbParams.labels.map_controls,
		'type': 'checkbox',
		'value': function( element ) {
	...
	{
		'name': nhbParams.labels.map_marker,
		'type': 'checkbox',
		'value': function( element ) {
...
window.nhbAddInspector( 'Map', {
 	'label': nhbParams.labels.map,
	'footer': nhbParams.is_lite ? nhbParams.labels.map_lite_footer : '',
 	'options': options
} );
...
window.nhbAddInspector( 'Tabs', {
	'label': nhbParams.labels.tabs,
	'footer': nhbParams.is_lite ? nhbParams.labels.tabs_lite_footer : '',
	'options': options
} );


/* globals ContentEdit, nhbParams */
 ...
 */
for ( var i = 0; i < window.nhbElementsWithInspector.length; i++ ) {
	if ( ContentEdit[ window.nhbElementsWithInspector[ i ] ] ) {
		window.nhbAddInspector( window.nhbElementsWithInspector[ i ], {
			'options': options
		} );
	}
}
...
(function() {
	...
	ContentTools.ToolUI.prototype.disabled = function(disabledState) {
		if ( this._domElement.classList.contains( 'nhb-tool-large' ) ) {
			return;
		}
...
(function() {
   ...
   ContentTools.ToolUI.prototype.mount = function( domParent, before ) {
	   ...
	   if ( this.tool.buttonName ) {
		   this._domElement.classList.add( 'nhb-tool-large' );
		   var label = document.createElement( 'div' );
		   label.classList.add( 'nhb-tool-title' );
		   label.textContent = this.tool.buttonName;
		   this._domElement.appendChild( label );
		   //
		//    if ( this.tool.premium && nhbParams.is_lite ) {
		// 	   this._domElement.classList.add( 'nhb-tool-is-premium' );
		// 	   var star = document.createElement( 'div' );
		// 	   star.classList.add( 'nhb-tool-premium' );
		...
		// 		   var preview = document.createElement( 'DIV' );
		// 		   preview.innerHTML = wp.template( 'nhb-preview-premium-element' )( {
		// 			   'title': this.tool.title || this.tool.icon,
		// 			   'description': this.tool.description,
		// 			   'image': nhbParams.plugin_url + 'no_hassle_builder/images/preview-' + this.tool.icon + '.gif'
		// 		   } );
...
/* globals ContentTools, ContentEdit, HTMLString, nhbParams */
...
ContentTools.Tool.refreshTooltip = function( value ) {
	...
	var buttonElement = window.nhbEditor.getToolUI( this.icon )._domElement;
...
(function() {
	...
	ContentTools.Tools.Table.buttonName = nhbParams.labels.table;
	ContentTools.Tools.Table.label = nhbParams.labels.table;
...
ContentTools.Tools.AddElementButton = (function(_super) {
    ...
    AddElementButton.isApplied = function() {
		if ( this.listTool ) {
			var tool = window.nhbEditor.getToolUI( this.listTool );
			if ( tool ) {
...
ContentTools.Tools.AddElementList = (function(_super) {
	...
	AddElementList.init = function() {
		...
		if ( ! toolDomElement.children.length ) {
			...
			elem.classList.add( 'nhb-add-element-list-none-found' );
	...
	AddElementList._onClick = function(e) {
		...
		if ( root.focused() ) {
			...
			while ( ! currTarget.classList.contains( 'nhb-add-element-list-item' ) && currTarget.parentNode ) {
	...
	AddElementList._doSearch = function( ev ) {
		var elements,
			...
			noneFoundLabel = this._ceElement._domElement.querySelector( '.nhb-add-element-list-none-found' );
	...
	ContentTools.ToolUI.prototype._addDOMEventListeners = function() {
		...
		if ( this.tool.__super__ && this.tool.__super__.constructor.name === 'AddElementButton' ) {
	        ...
	        this._domElement.addEventListener('click', function(e) {
				if ( e.target.classList.contains('ct-tool') ) {
					var tool = window.nhbEditor.getToolUI( this.tool.listTool );
...
/* globals ContentTools, ContentEdit, nhbInspectorOptions, __extends, nhbEditor, nhbParams */
...
ContentTools.Tools.Shortcode = (function(_super) {
	...
	Shortcode.label = nhbParams.labels.shortcode;

	Shortcode.icon = 'shortcode';

	Shortcode.tagName = 'shortcode';

	Shortcode.buttonName = nhbParams.labels.shortcode;
	...
	Shortcode.createNew = function( parent, index ) {
		...
		nhbEditor.shortcodeFrame.open({
			title: nhbParams.labels.insert_shortcode,
			button: nhbParams.labels.insert_shortcode,
			successCallback: function( view ) {
				...
				var isMapped = nhbParams.shortcode_mappings[ base ];
				var elem;
	...
	Shortcode.createInsertedShortcode = function( base ) {
		...
		// If there is an existing shortcode mapping, use that.
		if ( typeof nhbInspectorOptions.Shortcode[ base ] === 'undefined' ) {
			if ( typeof nhbParams.shortcode_mappings !== 'undefined' && typeof nhbParams.shortcode_mappings[ base ] !== 'undefined' ) {
				var editor = ContentTools.EditorApp.get();
		...
		if ( nhbInspectorOptions.Shortcode[ base ] && nhbInspectorOptions.Shortcode[ base ].options ) {
			for ( var i = 0; i < nhbInspectorOptions.Shortcode[ base ].options.length; i++ ) {
				var option = nhbInspectorOptions.Shortcode[ base ].options[ i ];
				if ( option.id ) {
...
/* globals ContentTools, __extends, ContentEdit, nhbParams */
...
ContentTools.Tools.Heading1 = (function(_super) {
	...
	Heading1.label = nhbParams.labels.heading_label.replace( '%d', '1' );
...
ContentTools.Tools.Heading2 = (function(_super) {
	...
	Heading2.label = nhbParams.labels.heading_label.replace( '%d', '2' );
...
ContentTools.Tools.Heading3 = (function(_super) {
	...
	Heading3.label = nhbParams.labels.heading_label.replace( '%d', '3' );
...
ContentTools.Tools.Heading4 = (function(_super) {
	...
	Heading4.label = nhbParams.labels.heading_label.replace( '%d', '4' );
...
ContentTools.Tools.Heading5 = (function(_super) {
	...
	Heading5.label = nhbParams.labels.heading_label.replace( '%d', '5' );
...
ContentTools.Tools.Heading6 = (function(_super) {
	...
	Heading6.label = nhbParams.labels.heading_label.replace( '%d', '6' );
...
/* globals ContentTools, __extends, nhbParams */
...
ContentTools.Tools.Blockquote = (function(_super) {
  ...
  Blockquote.label = nhbParams.labels.blockquote;
...
ContentTools.Tools.ClearFormatting = (function(_super) {
  ...
  ClearFormatting.label = nhbParams.labels.clear_formatting;
...
/* globals ContentTools, HTMLString, __extends, nhbParams */
...
ContentTools.Tools.Code = (function(_super) {
	...
	Code.label = nhbParams.labels.code;
...
ContentTools.Tools.Color = (function(_super) {
	...
	Color.label = nhbParams.labels.color;
	...
	Color.canApply = function(element, selection) {
		...
		if ( color ) {
			window.nhbEditor.getToolUI( this.icon )._domElement.setAttribute('data-tooltip', this.label + ': ' + color );
		} else {
			window.nhbEditor.getToolUI( this.icon )._domElement.setAttribute('data-tooltip', this.label );
		}
	...
	Color.apply = function(element, selection, callback) {
		...
		wp.hooks.doAction( 'nhb.tool.color.applied', element );
...
ContentEdit.Root.get().bind('blur', function() {
	var tool = window.nhbEditor.getToolUI( 'color' );
	...
	if ( this.tool.name === 'Color' ) {
        ...
        this._domElement.addEventListener('click', function(e) {
			if ( e.target.classList.contains('ct-tool') ) {
				...
				Array.prototype.forEach.call(elements, function(el){
					if ( typeof el._nhbInitDone === 'undefined' ) {
						el._nhbInitDone = true;
...
/* globals ContentEdit, ContentTools, __extends, nhbParams, nhbEditor */
...
ContentTools.Tools.TwoColumn = (function(_super) {
	...
	OneColumn.label = nhbParams.labels.one_column;

	OneColumn.icon = 'onecolumn';

	OneColumn.tagName = 'onecolumn';

	OneColumn.buttonName = nhbParams.labels.one_column;
	...
	OneColumn.createNew = function( parent, index ) {
		...
		wp.hooks.doAction( 'nhb.tool.row.applied', row );
	};
...
ContentTools.Tools.TwoColumn = (function(_super) {
	...
	TwoColumn.label = nhbParams.labels.two_column;

	TwoColumn.icon = 'twocolumn';

	TwoColumn.tagName = 'twocolumn';

	TwoColumn.buttonName = nhbParams.labels.two_column;
...
ContentTools.Tools.ThreeColumn = (function(_super) {
	...
	ThreeColumn.label = nhbParams.labels.three_column;

	ThreeColumn.icon = 'threecolumn';

	ThreeColumn.tagName = 'threecolumn';

	ThreeColumn.buttonName = nhbParams.labels.three_column;
...
ContentTools.Tools.FourColumn = (function(_super) {
	...
	FourColumn.label = nhbParams.labels.four_column;

	FourColumn.icon = 'fourcolumn';

	FourColumn.tagName = 'fourcolumn';

	FourColumn.buttonName = nhbParams.labels.four_column;
...
(function() {
	...
	ContentTools.ToolUI.prototype.mount = function( domParent, before ) {
		...
		if ( this._domElement.classList.contains( 'ct-tool--insert-element' ) ) {
			...
			this._domElement.innerHTML = '<span>' + nhbParams.labels.add_element + '</span>';
...
ContentTools.Tools.AlignJustify = (function(_super) {
	...
	AlignJustify.label = nhbParams.labels.justify;
...
/* globals ContentEdit, ContentTools, __extends, nhbParams */
...
ContentTools.Tools.Sidebar = (function(_super) {
	...
	Sidebar.label = nhbParams.labels.sidebar_or_widget_area;

	Sidebar.icon = 'sidebar';

	Sidebar.tagName = 'sidebar';

	Sidebar.buttonName = nhbParams.labels.sidebar;
	...
	Sidebar.createNew = function( parent, index ) {
		var defaultSidebar = '';
		for ( var sidebarID in nhbParams.sidebar_list ) {
			if ( sidebarID ) {
				if ( nhbParams.sidebar_list.hasOwnProperty( sidebarID ) ) {
		...
		var shortcode = wp.shortcode.next( 'nhb_sidebar', '[nhb_sidebar id="' + defaultSidebar + '"][/nhb_sidebar]', 0 );
...
ContentTools.Tools.Hr = (function(_super) {
	...
	Hr.label = nhbParams.labels.horizontal_rule;
...
ContentTools.Tools.Icon = (function(_super) {
	...
	Icon.label = nhbParams.labels.icon;

	Icon.icon = 'icon';

	Icon.tagName = 'icon';

	Icon.buttonName = nhbParams.labels.icon;

	Icon.apply = function(element, selection, callback) {

		nhbEditor.iconFrame.open({
			title: nhbParams.labels.insert_icon,
			button: nhbParams.labels.use_icon,
			successCallback: function( view ) {
 ...
 */
ContentTools.Tools.Link._nhbToolLinkOverrideCanApply = ContentTools.Tools.Link.canApply;
ContentTools.Tools.Link.canApply = function(element, selection) {
	var ret = this._nhbToolLinkOverrideCanApply( element, selection );
	if ( ! ret ) {
...
window.addEventListener( 'DOMContentLoaded', function() {
	document.addEventListener( 'click', function(ev) {
		if ( ev.target.getAttribute( 'id' ) === 'wp-link-submit' ) {
			...
			var element = window._nhbCurrentLink.element,
				selection = window._nhbCurrentLink.selection,
				existingClass = window._nhbCurrentLink.existingClass,
				existingStyles = window._nhbCurrentLink.existingStyles,
				linkType = window._nhbCurrentLink.type;
			...
			if ( url ) {
				...
				args = wp.hooks.applyFilters( 'nhb.tool.' + linkType.toLowerCase() + '.args', args );
			...
			delete window._nhbCurrentLink;
			wp.hooks.doAction( 'nhb.tool.' + linkType.toLowerCase() + '.applied', element, from, to );
...
/* globals ContentEdit, ContentTools, __extends, nhbEditor, nhbParams */
...
ContentTools.Tools.Widget = (function(_super) {
	...
	Widget.label = nhbParams.labels.widget;

	Widget.icon = 'widget';

	Widget.tagName = 'widget';

	Widget.buttonName = nhbParams.labels.widget;
	...
	Widget.createNew = function( parent, index ) {
		nhbEditor.widgetFrame.open({
			title: nhbParams.labels.insert_widget,
			button: nhbParams.labels.insert_widget,
			successCallback: function( view ) {
				var base = 'nhb_widget';
				var widgetSlug = view.selected.getAttribute( 'data-widget-slug' );
				var shortcodeRaw = '[nhb_widget widget="' + widgetSlug + '" ]';
				var shortcode = wp.shortcode.next( base, shortcodeRaw, 0 );
...
ContentTools.Tools.Text = ( function( _super ) {
	...
	Text.label = nhbParams.labels.text;
	...
	Text.buttonName = nhbParams.labels.text;
...
ContentTools.Tools.Html = (function(_super) {
	...
	Html.label = nhbParams.labels.html;

	Html.icon = 'html';

	Html.tagName = 'html';

	Html.buttonName = nhbParams.labels.html;
...
ContentTools.Tools.Map = ( function( _super ) {
	...
	Map.label = nhbParams.labels.map;
	...
	Map.buttonName = nhbParams.labels.map;
...
ContentTools.Tools.Tabs = ( function( _super ) {
	...
	Tabs.label = nhbParams.labels.tabs;

	Tabs.icon = 'tabs';

	Tabs.tagName = 'tabs';

	Tabs.buttonName = nhbParams.labels.tabs;
	...
	Tabs.apply = function(element, selection, callback) {
		...
		if ( window.nhbSelectorMatches( element._domElement, '[data-ce-tag="tabs"] *' ) ) {
	...
	Tabs.createNew = function( parent, index ) {
		...
		while ( hashes.length < 4 ) {
			var hash = window.nhbEditor.generateHash();
			if ( document.querySelector( '.nhb-tabs-' + hash ) ) {
				continue;
			}
			if ( document.querySelector( '[id="nhb-tabs-' + hash + '"]' ) ) {
				continue;
			}
		...
		elem.classList.add( 'nhb-tabs-' + hashes[0] );
		elem.setAttribute( 'data-ce-tag', 'tabs' );

		/* jshint multistr: true */
		elem.innerHTML =
			'<input class="nhb-tab-state" type="radio" name="nhb-tabs-' + hashes[0] + '" id="nhb-tab-' + hashes[1] + '" data-tab="1" data-ce-tag="tabradio" checked />' +
			'<input class="nhb-tab-state" type="radio" name="nhb-tabs-' + hashes[0] + '" id="nhb-tab-' + hashes[2] + '" data-tab="2" data-ce-tag="tabradio" />' +
			'<input class="nhb-tab-state" type="radio" name="nhb-tabs-' + hashes[0] + '" id="nhb-tab-' + hashes[3] + '" data-tab="3" data-ce-tag="tabradio" />' +
			'<div class="nhb-tab-tabs" data-ce-tag="tabcontainer">' +
		        '<label for="nhb-tab-' + hashes[1] + '" data-ce-tag="tab" class="nhb-tab-active"><span style="font-weight: bold;">Tab 1</span></label>' +
		        '<label for="nhb-tab-' + hashes[2] + '" data-ce-tag="tab"><span style="font-weight: bold;">Tab 2</span></label>' +
		        '<label for="nhb-tab-' + hashes[3] + '" data-ce-tag="tab"><span style="font-weight: bold;">Tab 3</span></label>' +
		    '</div>' +
			'<div class="nhb-tab-panels" data-ce-tag="tabpanelcontainer">' +
				'<div class="nhb-row" data-panel="1"><div class="nhb-col"><p>Tab 1 content</p></div></div>' +
				'<div class="nhb-row" data-panel="2"><div class="nhb-col"><p>Tab 2 content</p></div></div>' +
				'<div class="nhb-row" data-panel="3"><div class="nhb-col"><p>Tab 3 content</p></div></div>' +
			'</div>';
...
/* globals ContentTools, ContentEdit, ContentSelect, __extends, nhbParams */

// The Media Manager window needs to know our post ID so that Insert from URL will work.
if ( wp.media.view.settings.post ) {
	wp.media.view.settings.post.id = nhbParams.post_id;
}

ContentTools.Tools.nhbMedia = (function(_super) {
	__extends(nhbMedia, _super);

	function nhbMedia() {
		return nhbMedia.__super__.constructor.apply(this, arguments);
	}

	ContentTools.ToolShelf.stow(nhbMedia, 'nhb-media');

	nhbMedia.label = nhbParams.labels.media;

	nhbMedia.icon = 'image';

	nhbMedia.buttonName = nhbParams.labels.media;

	nhbMedia.apply = function(element, selection) {
		if ( this._isOpen() ) {
			return;
		}
		...
		window._nhbAddMediaOrigInsert = wp.media.editor.insert;
		wp.media.editor.insert = this.nhbAddMediaOverrideInsert.bind( this );

		wp.media.editor.open();
	};

	nhbMedia.createNew = function( parent, index ) {
	...
	nhbMedia.nhbAddMediaOverrideInsert = function( html ) {
		...
		wp.media.editor.insert = window._nhbAddMediaOrigInsert;
...
(function() {
	var dragEnterHandler = function() {
		...
		var elem = root.focused();
		if ( elem ) {
			window.nhbEditor.getToolUI( 'nhb-media' ).apply( elem, null );
		}
	};
...
ContentTools.Tools.Underline = (function(_super) {
  ...
  Underline.label = nhbParams.labels.underline;
...
ContentTools.Tools.paragraphPicker = (function(_super) {
	...
	paragraphPicker.label = nhbParams.labels.text_style;
	...
	paragraphPicker.types = {
		p: { className: 'Paragraph', label: nhbParams.labels.paragraph },
		h1: { className: 'Heading1', label: nhbParams.labels.heading_1 },
		h2: { className: 'Heading2', label: nhbParams.labels.heading_2 },
		h3: { className: 'Heading3', label: nhbParams.labels.heading_3 },
		h4: { className: 'Heading4', label: nhbParams.labels.heading_4 },
		h5: { className: 'Heading5', label: nhbParams.labels.heading_5 },
		h6: { className: 'Heading6', label: nhbParams.labels.heading_6 },
		blockquote: { className: 'Blockquote', label: '"' + nhbParams.labels.blockquote + '"' },
		pre: { className: 'Preformatted', label: nhbParams.labels.preformatted }
	};
	...
	paragraphPicker.canApply = function(element) {
		if ( this.types.hasOwnProperty( element.tagName() ) ) {
			for ( var tag in this.types ) {
				...
				if ( this._ceElement._domElement.classList.contains( 'nhb-paragraph-picker-type-' + tag ) ) {
					this._ceElement._domElement.classList.remove( 'nhb-paragraph-picker-type-' + tag );
				}
			}
			if ( ! this._ceElement._domElement.classList.contains( 'nhb-paragraph-picker-type-' + element.tagName() ) ) {
				this._ceElement._domElement.classList.add( 'nhb-paragraph-picker-type-' + element.tagName() );
	...
	paragraphPicker.isApplied = function() {
		return ! this._ceElement._domElement.classList.contains( 'nhb-paragraph-picker-type-p' );
	};
	...
	paragraphPicker._paragraphPickerMount = function() {
		...
		this._ceElement._domElement.innerHTML = nhbParams.labels.paragraph;
		this._ceElement._domElement.appendChild(d);
...
ContentEdit.Root.get().bind('blur', function() {
	var tool = window.nhbEditor.getToolUI( 'paragraph' );
...
ContentTools.Tools.Strikethrough = (function(_super) {
  ...
  Strikethrough.label = nhbParams.labels.strikethrough;
...
/* globals nhbEditor, ContentTools, ContentEdit, nhbParams */
...
nhbEditor.shortcuts = {
	...
	'ctrl+m': function( element, selection, callback ) {
		...
		ContentTools.Tools.nhbMedia.apply( element, selection, callback );
...
nhbEditor.shortcuts[ redoKey ] = function() {
...
nhbEditor._shortcuts = {};

for ( var code in nhbEditor.shortcuts ) {

	if ( nhbEditor.shortcuts.hasOwnProperty( code ) ) {
		...
		nhbEditor._shortcuts[ key ] = nhbEditor.shortcuts[ code ];

	}

}

nhbEditor.shortcuts = nhbEditor._shortcuts;

nhbEditor._shortcuts = null;
...
(function() {
	...
	var shortcutListener = function(ev) {
		...
		if ( wp.hooks.applyFilters( 'nhb.shortcuts', false, key, element ) ) {
		...
		if ( nhbEditor.shortcuts[ key ] ) {
			...
			nhbEditor.shortcuts[ key ]( element, selection, function() {} );
...
document.addEventListener( 'DOMContentLoaded', function() {
	...
	var goPremiumButton = document.querySelector( '#wp-admin-bar-nhb_go_premium' );
	...
	if ( goPremiumButton ) {
		...
		goPremiumButton.addEventListener( 'click', function() {
			...
			div.innerHTML = wp.template( 'nhb-learn-premium' )();

			div.setAttribute( 'id', 'nhb-learn-premium-wrapper' );
			...
			document.querySelector( '#nhb-learn-premium-wrapper .nhb-tour-close' ).addEventListener( 'click', function() {
			...
			var carousel = document.querySelector( '.nhb-learn-carousel' );
			...
			if (carousel) {
			    ...
			    var slides = carousel.querySelectorAll('.nhb-learn-slide');

			    var indicators = carousel.querySelectorAll('.nhb-learn-indicators label');
 ...
 */
window.nhbPlayTour = function() {

	// If the tour has started before, possible from another page, don't show it again.
	// Helpful for the nhb demo site.
	if ( localStorage ) {
		if ( localStorage.getItem( 'nhb_did_intro' ) ) {
			return;
		}
		localStorage.setItem( 'nhb_did_intro', 1 );
	}

	var tourModalWrapper = document.createElement( 'DIV' );
	tourModalWrapper.classList.add( 'nhb-tour-modal-wrapper' );

	var tourModal = document.createElement( 'DIV' );
	tourModal.classList.add( 'nhb-tour-modal' );
	tourModalWrapper.appendChild( tourModal );

	var closeButton = document.createElement( 'DIV' );
	closeButton.classList.add( 'nhb-tour-close' );
	tourModal.appendChild( closeButton );

	closeButton.addEventListener( 'click', function() {
		tourModalWrapper.classList.remove( 'nhb-tour-shown' );
		setTimeout( function() {
	...
	setTimeout( function() {
		tourModalWrapper.classList.add( 'nhb-tour-shown' );
	}, 100 );

	var editor = ContentTools.EditorApp.get();
	editor.unbind( 'start', window.nhbPlayTour );
};
 ...
 */
wp.hooks.addAction( 'nhb.ct.ready', function() {
	if ( ! nhbParams.do_intro ) {
		return;
	}

	var editor = ContentTools.EditorApp.get();
	editor.bind( 'start', window.nhbPlayTour );
});

/* globals nhbParams */

/**
 * Asks the user to rate nhb.
 */
window.addEventListener( 'DOMContentLoaded', function() {
	...
	jQuery( document ).on( 'heartbeat-tick', function( e, data ) {
		...
		if ( data.ask_for_rating ) {
			document.querySelector( '#nhb-rate-desc' ).innerHTML = data.ask_for_rating;
			document.querySelector( '#wp-admin-bar-nhb_rate' ).classList.remove( 'nhb-hidden' );
			document.querySelector( '#nhb-rate-no' ).addEventListener( 'click', function( ev ) {
				ev.preventDefault();
				document.querySelector( '#wp-admin-bar-nhb_rate' ).classList.add( 'nhb-hidden' );
			} );
			document.querySelector( '#nhb-rate-go' ).addEventListener( 'click', function( ev ) {
				ev.preventDefault();
				document.querySelector( '#wp-admin-bar-nhb_rate' ).classList.add( 'nhb-hidden' );

				var payload = new FormData();
				payload.append( 'action', 'nhb_ask_rating_rated' );
				payload.append( 'nonce', nhbParams.nonce );
...
/**
 * Send usage stats to nhb.com if opted in.
 *
 * @since 2.11
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	if ( nhbParams.tracking_opted_in ) {
		wp.hooks.addAction( 'nhb.save.payload.post', function( payload ) {
		    var xhrTracking = new XMLHttpRequest();
		    xhrTracking.open( 'POST', nhbParams.ajax_url );
			payload.append( 'action', 'nhb_save_content_tracking' );
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	jQuery( document ).on( 'heartbeat-send', function( e, data ) {
		...
		data['wp-refresh-post-nonces'] = {
			post_id: nhbParams.post_id
		};

		// Refresh our own nonce regularly.
		data.nhb_nonce = nhbParams.nonce;

		// Refresh our media manager nonce (this is difference from our nhb nonce).
		data.media_manager_editor_nonce = wp.media.view.settings.nonce.sendToEditor;

		// Needed for nonces and post locking.
		data.post_id = nhbParams.post_id;
		...
		if ( nhbParams.autosave_interval ) {
			autosaveInterval = parseInt( nhbParams.autosave_interval, 10 ) * 60;
		}
		if ( autosaveDiff > autosaveInterval ) {
			...
			data.content = wp.hooks.applyFilters( 'nhb.save', data.content );
		}
	} );
	...
	jQuery( document ).on( 'heartbeat-tick', function( e, data ) {
		...
		if ( data['wp-refresh-post-nonces'] ) {
			...
			// Update the nhb nonce if invalid already.
			if ( nonces.nhb_nonce_new ) {
				nhbParams.nonce = nonces.nhb_nonce_new;
			}
		...
		// Update the nhb nonce if invalid already.
		if ( data.nhb_nonce_new ) {
			nhbParams.nonce = data.nhb_nonce_new;
		}
		...
		if ( data.has_post_lock ) {
			...
			if ( ! document.querySelector( '#nhb-post-locked-dialog' ) ) {
				...
				var div = document.createElement( 'DIV' );
				div.innerHTML = wp.template( 'nhb-heartbeat-takeover' )( templateData );
				div.setAttribute( 'id', 'nhb-post-locked-dialog' );
				document.body.appendChild( div );
				modalIsOpen = true;

				// If main button was clicked, reload the page.
				document.querySelector( '.nhb-post-takeover-refresh' ).addEventListener( 'click', function( ev ) {
					ev.preventDefault();
					modalIsOpen = false;
	...
	/**
	 * Triggers a post lock check & nhb nonce check.
	 */
	function checkHeartbeat() {
	    var payload = new FormData();
		payload.append( 'action', 'nhb_heartbeat_check' );
		payload.append( 'post_id', nhbParams.post_id );
		payload.append( 'nonce', nhbParams.nonce );
	   		...
	   		if ( xhr.status >= 200 && xhr.status < 400 ) {
				...
				if ( response ) {
					...
					if ( response.post_lock ) {
						...
						var div = document.createElement( 'DIV' );
						div.innerHTML = wp.template( 'nhb-heartbeat-locked' )( data );
						div.setAttribute( 'id', 'nhb-post-locked-dialog' );
						document.body.appendChild( div );
						modalIsOpen = true;

						// Cancel / back handler.
						document.querySelector( '.nhb-post-locked-back' ).addEventListener( 'click', function( ev ) {
							ev.preventDefault();
							modalIsOpen = false;
							doRemovePostLock = false;
							document.querySelector( '#wp-admin-bar-gambit_builder_cancel' ).dispatchEvent( new CustomEvent( 'click' ) );
						...
						// Takeover handler.
						document.querySelector( '.nhb-post-locked-takeover' ).addEventListener( 'click', function( ev ) {
							ev.preventDefault();
							modalIsOpen = false;
							overridePostLock();
							div.parentNode.removeChild( div );
					...
					// Update the nhb nonce if given.
					if ( response.nonce ) {
						nhbParams.nonce = response.nonce;
					}
	...
	function removePostLock() {
	    var payload = new FormData();
		payload.append( 'action', 'nhb_remove_post_lock' );
		payload.append( 'post_id', nhbParams.post_id );
		payload.append( 'nonce', nhbParams.nonce );
	...
	function overridePostLock() {
	    var payload = new FormData();
		payload.append( 'action', 'nhb_override_post_lock' );
		payload.append( 'post_id', nhbParams.post_id );
		payload.append( 'nonce', nhbParams.nonce );
	...
	function doAutosave() {
	    var payload = new FormData();
		payload.append( 'action', 'nhb_autosave' );
		payload.append( 'post_id', nhbParams.post_id );
		payload.append( 'nonce', nhbParams.nonce );

		// Get the content & do the normal filters.
		var content = ContentTools.EditorApp.get().regions()['main-content'].html();
		content = wp.hooks.applyFilters( 'nhb.save', content );
 ...
 */
window.nhbEditor.hexToRgb = function( hex ) {
 ...
 */
window.nhbEditor.rgbToHsl = function( r, g, b ) {
    r /= 255, g /= 255, b /= 255;
 ...
 ***************************************************************************/
window.nhbEditor.formattingTools = [[]];
window.nhbEditor.formattingTools[0].push(
	'insertElement',
	'paragraphPicker',
	'|',
	'color',
...
window.nhbEditor.insertElements = [[]];
window.nhbEditor.insertElements[0].push(
	'onecolumn',
	'twocolumn',
	'threecolumn',
	'fourcolumn',
...
// No longer needed.
window.nhbEditor.toolHeadings = [];

// window.nhbEditor.advancedTools = [
// 	'h3', 'h4', 'h5', 'h6', 'table', 'preformatted', 'indent', 'unindent', 'code', 'align-justify', 'uppercase', 'strikethrough',
// 	'nhb-advanced-formatting-group',
// 	'nhb-shortcodes-group'
// 	//, 'nhb-rows-columns-group'
// ];

window.addEventListener( 'DOMContentLoaded', function() {
	// new nhbEditor.MarginBottom();
	// new nhbEditor.MarginTop();
	// new nhbEditor.MarginBottomContainer();
	// new nhbEditor.MarginTopContainer();
	// new nhbEditor.OverlayColumnWidth();
	// new nhbEditor.OverlayColumnWidthRight();
	// new nhbEditor.OverlayColumnWidthLabels();
	new nhbEditor.OverlayColumn();
	new nhbEditor.OverlayRow();
	new nhbEditor.OverlayElement();
	// new nhbEditor.OverlayResize();

	new nhbEditor.ToolbarElement();
	new nhbEditor.ToolbarImage();
	new nhbEditor.ToolbarHtml();
	new nhbEditor.ToolbarIframe();
	new nhbEditor.ToolbarEmbed();
	new nhbEditor.ToolbarMap();
	new nhbEditor.ToolbarIcon();
	new nhbEditor.ToolbarShortcode();
	new nhbEditor.ToolbarRow();
	new nhbEditor.ToolbarColumn();
	new nhbEditor.ToolbarNewsletter();
	new nhbEditor.ToolbarList();
	new nhbEditor.ToolbarTabContainer();
	new nhbEditor.ToolbarTable();
	new nhbEditor.ToolbarTitle();
	if ( ! nhbParams.is_lite ) {

	}

	new nhbEditor.TooltipLink();
	new nhbEditor.TooltipButton();
	new nhbEditor.TooltipInput();
	new nhbEditor.TooltipTab();
});
