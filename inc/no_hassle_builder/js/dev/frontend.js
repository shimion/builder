...
window.nhbIsRTL = function() {
	var html = document.querySelector('html');
	return html.getAttribute( 'dir' ) === 'rtl';
};

window._nhbFixRowWidth = function( element ) {

	var dataWidth = element.getAttribute('data-width');

	// Nested rows cannot be full width
	if ( element.parentNode.classList.contains('nhb-col') ) {
		window._nhbRowReset( element );
	} else if ( typeof dataWidth === 'undefined' || ! dataWidth ) {
		window._nhbRowReset( element );
	} else if ( dataWidth === 'full-width' ) {
		window._nhbFullWidthRow( element );
	} else {
		window._nhbFullWidthRow( element, true );
	}

	clearTimeout( window._nhbFixRowWidthsResizeTrigger );
	window._nhbFixRowWidthsResizeTrigger = setTimeout( function() {
		window._nhbFixRowWidthsResizeNoReTrigger = true;
		window.dispatchEvent( new CustomEvent( 'resize' ) );
	}, 1 );
};


window._nhbRowReset = function( element ) {
	element.style.width = '';
...
window._nhbFullWidthRow = function( element, fitToContentWidth ) {
...
window.nhbFixRowWidths = function() {
	var fullRows = document.querySelectorAll('.nhb-row[data-width]');
	Array.prototype.forEach.call(fullRows, function(el){
		window._nhbFixRowWidth( el );
	});
};

window.addEventListener('resize', function() {
	if ( window._nhbFixRowWidthsResizeNoReTrigger ) {
		delete window._nhbFixRowWidthsResizeNoReTrigger;
		return;
	}
...
(function() {
	...
	window.nhbInitAllPretext = function() {

		if ( typeof hljs === 'undefined' ) {
			return;
		}
		var codes = document.querySelectorAll( '.nhb-main-wrapper pre' );
	...
	window.addEventListener( 'DOMContentLoaded', window.nhbInitAllPretext );
})();


window.nhbTabsRefreshActiveTab = function( tabsElement ) {
	var radio = tabsElement.querySelector( '.nhb-tab-state:checked' );
	var id = radio.getAttribute( 'id' );
	var tabs = tabsElement.querySelector( '.nhb-tab-tabs ' );
	if ( tabs ) {
		var activeTab = tabs.querySelector( '.nhb-tab-active' );
		if ( activeTab ) {
			activeTab.classList.remove( 'nhb-tab-active' );
		}
		activeTab = tabs.querySelector( '[for="' + id + '"]' );
		if ( activeTab ) {
			activeTab.classList.add( 'nhb-tab-active' );
		}
	}
};
...
( function() {
	...
	window.addEventListener( 'DOMContentLoaded', function() {
		...
		document.addEventListener( 'change', function( ev ) {
			if ( ev.target ) {
				if ( ev.target.classList.contains( 'nhb-tab-state' ) ) {
					window.nhbTabsRefreshActiveTab( ev.target.parentNode );
				}
			}
		} );
	...
	document.addEventListener( 'DOMContentLoaded', function() {
		...
		Array.prototype.forEach.call( elements, function( el ) {
			...
			if ( el ) {
				el.classList.add( 'nhb-tab-active' );
			}
		} );
	} );
