/* globals initnhbMaps, google, nhbEditor */
...
var nhbMapsInitialized = [];

initnhbMaps = function( mapElem, successCallback, failCallback ) { // jshint ignore:line

	// Cleanup all other previously initialized maps.
	var cleanup = function() {
		for ( var i = nhbMapsInitialized.length - 1; i >= 0; i-- ) {
			var elem = nhbMapsInitialized[ i ].getDiv();
			while ( elem.parentNode && elem.parentNode.tagName !== 'BODY' ) {
				elem = elem.parentNode;
			}
			if ( ! elem.parentNode ) {
				if ( nhbMapsInitialized[ i ].marker ) {
					nhbMapsInitialized[ i ].marker.setMap( null );
					delete( nhbMapsInitialized[ i ].marker );
				}
				delete( nhbMapsInitialized[ i ] );
				nhbMapsInitialized.splice( i, 1 );
			}
		}
	};
 ...
 */
window.nhbMapsReCenter = function( mapElem ) {
	if ( typeof google !== 'undefined' ) {
		...
		for ( var i = 0; i < nhbMapsInitialized.length; i++ ) {
			window.nhbMapsReCenter( nhbMapsInitialized[ i ].getDiv() );
		}
	}
};
...
window.addEventListener( 'DOMContentLoaded', function() {
	window.addEventListener( 'resize', window.nhbMapsReCenter );
} );
 ...
 */
 window.addEventListener( 'DOMContentLoaded', function() {
 	document.onclick = function( event ) {
		var el = event.target;
		if ( el.parentNode && el.parentNode.classList && el.parentNode.classList.contains( 'nhb-tab-tabs' ) ) {
