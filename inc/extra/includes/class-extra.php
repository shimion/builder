...
//			add_filter( 'auto_update_plugin', '_include_plugins_in_auto_update', 10, 2 );
		...
		static function add_debug_page() {
			...
			$title = sprintf( '%s [v.%s]', __fs( 'extra-debug' ), WP_FS__SDK_VERSION );
						...
						$message = sprintf(
							...
							sprintf(
								...
								sprintf(
									...
									' - ' . __fs( 'deactivate-plugin-desc', 'extra', $this->_slug )
								)
							)
						);
						break;
							...
							sprintf(
								...
								sprintf(
									...
									sprintf(
										...
										'<b>' . implode( ', ', $this->apply_filters( 'api_domains', array( 'api.extra.com' ) ) ) . '</b>' )
								),
								sprintf(
									'<a href="%s"><b>%s</b></a>%s',
									wp_nonce_url( 'plugins.php?action=deactivate&amp;plugin=' . $this->_plugin_basename . '&amp;plugin_status=' . 'all' . '&amp;paged=' . '1' . '&amp;s=' . '', 'deactivate-plugin_' . $this->_plugin_basename ),
		...
		private function should_stop_execution() {
			...
			if ( $this->is_activation_mode() ) {
				...
				if ( ! WP_FS__IS_HTTP_REQUEST ) {
					 ...
					 * @since  1.1.6.3
					 *
					 * @link   https://wordpress.org/support/topic/errors-in-the-extra-class-when-running-in-wordpress-in-cli
					 */
					return true;
				}
...
//			$this->sync_install( array(), true );
		 ...
		 *      If you are interested in custom event tracking please contact yo@extra.com
		 *      for further details.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.2.1
